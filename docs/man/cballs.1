...
[a: mcheb] is the total number of chebyshev multipoles to use. (mChebyshev + 1) must be a power of 2.

.IP "\fBnsmooth\fR" 12
[a: nsm] Number of bodies to smooth out (or in a bucket).

.IP "\fBrsmooth\fR" 12
[a: rsm] Radius of the pivot smoothing neighbourhood.

.IP "\fBtheta\fR" 12
Control tree search parameter, can be used to increase speed.

.IP "\fBcomputeTPCF\fR" 12
[a: tpcf] If true, compute 3pcf.

.IP "\fBusePeriodic\fR" 12
[a: tpcf] If false, don't use periodic boundary condition.

.IP "\fBinfile\fR" 12
[a: in]  gives the name of file to input the data to analyse.

.IP "\fBinfileformat\fR" 12
[a: infmt]  gives format of file to input the data to analyse. Common options are: 'columns-ascii', 'binary', 'takahasi'. First one is a file with a header and all the data point info is in column form: positions and convergence field. The binary format is a \fBcballs\fR binary format. Whereas takahasi format is for reading Takahasi simulations data.

.IP "\fBoutfile\fR" 12
[a: o] will give the name of file to save the analysed data. The output is written in column form by default, there is also a binary format. And by default this options is empty then no output file is saved.
...
[a: nthreads] you may give here various code behavior options.
