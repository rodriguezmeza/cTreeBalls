...
#    def getHistZetaM_sincos(self, int mChebyshev, int sizeHistN):
    def getHistZetaM_sincos(self):
        """
        Gives ZetaM matrix for a given mChebyshev and sizeHistN

        .. note::

            No note at this moment...

        """
        cdef int index_m
        cdef int index_n1
        cdef int index_n2
        cdef int mChebyshev = 7
        cdef int sizeHistN = 20
        cdef double ***outZM

        outZM = <double***> calloc(mChebyshev*sizeHistN*sizeHistN,sizeof(double))

        cdef np.ndarray[DTYPE_t, ndim=3] out_Z = np.zeros((mChebyshev, sizeHistN, sizeHistN), 'float64')

        if saveHistZetaM_sincos(&self.cmd, &self.gd, mChebyshev, sizeHistN, outZM)==FAILURE:
            raise cBallsSevereError(self.mgpt.error_message)

        for index_m in range(0, mChebyshev):
            for index_n1 in range(0, sizeHistN):
                for index_n2 in range(0, sizeHistN):
                    out_Z[index_m][index_n1][index_n2] = outZM[index_m][index_n1][index_n2]

        return out_Z

    def theta(self):
#        self.Run(["PrintParameterFile"])
        self.Run(["MainLoop"])
        print('theta=',self.cmd.theta)
        print('Rcut=',self.gd.Rcut)
        return self.cmd.theta
