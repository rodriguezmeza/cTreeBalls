...
cdef class cballs:
    ...
    cdef int computed # Flag to see if cyballs has already computed with the given pars
    cdef int allocated # Flag to see if cyballs structs are allocated already
...
#   when using cyballs in a python loop. Must be analysed and corrected
