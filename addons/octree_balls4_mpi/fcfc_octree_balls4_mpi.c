/* Deterministic MPI runtime for the native-octree BALLS4 estimator. */

#include <limits.h>
#include <stdlib.h>

#include "globaldefs.h"
#include "fcfc_octree_balls4_mpi.h"

#ifdef OCTREEBALLS4MPI

#define OCTREE_BALLS4_MPI_ROOT 0

#if defined(SINGLEPREC)
#define OCTREE_BALLS4_MPI_REAL MPI_FLOAT
#else
#define OCTREE_BALLS4_MPI_REAL MPI_DOUBLE
#endif

#ifdef LONGINT
#define OCTREE_BALLS4_MPI_INTEGER MPI_LONG
#else
#define OCTREE_BALLS4_MPI_INTEGER MPI_INT
#endif
...
int fcfc_octree_balls4_mpi_prepare(struct cmdline_data *cmd,
                                   struct global_data *gd)
{
    ...
    if (cmd->searchMethod == NULL
        ...
        return SUCCESS;
    if (mpi_active) {
        if (mpi_rank != OCTREE_BALLS4_MPI_ROOT) {
            cmd->verbose = 0;
            cmd->verbose_log = 0;
            gd->flagPrint = FALSE;
        }
    ...
    if (finalized) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 "octree-balls4-mpi cannot start after MPI_Finalize");
        return FAILURE;
    }
    ...
    if (!initialized) {
        ...
        if (atexit(finalize_at_exit) != 0) {
            snprintf(cmd->error_message, _ERRORMSGSIZE_,
                     "octree-balls4-mpi could not register MPI cleanup");
            finalize_at_exit();
            return FAILURE;
        }
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 "octree-balls4-mpi requires MPI_THREAD_FUNNELED support");
        return FAILURE;
    }
...
int fcfc_octree_balls4_mpi_finalize(struct cmdline_data *cmd)
{
    int finalized = FALSE;
    int status;
...
int fcfc_octree_balls4_mpi_active(void) { return mpi_active; }
int fcfc_octree_balls4_mpi_is_root(void)
{
    return !mpi_active || mpi_rank == OCTREE_BALLS4_MPI_ROOT;
}
int fcfc_octree_balls4_mpi_rank(void) { return mpi_rank; }
int fcfc_octree_balls4_mpi_size(void) { return mpi_size; }

int fcfc_octree_balls4_mpi_output_enabled(struct cmdline_data *cmd)
{
    return cmd->searchMethod == NULL
        || strcmp(cmd->searchMethod, "octree-balls4-mpi") != 0
        || !mpi_active || mpi_rank == OCTREE_BALLS4_MPI_ROOT;
}

int fcfc_octree_balls4_mpi_consensus(struct cmdline_data *cmd,
                                     int local_status,
...
int fcfc_octree_balls4_mpi_task_owned(INTEGER task)
{
    if (!mpi_active || mpi_size <= 1) return TRUE;
    return (int)(task % (INTEGER)mpi_size) == mpi_rank;
}

int fcfc_octree_balls4_mpi_reduce_reals(struct cmdline_data *cmd,
    ...
    while (offset < count) {
        ...
        if (mpi_rank == OCTREE_BALLS4_MPI_ROOT)
            status = MPI_Reduce(MPI_IN_PLACE, values + offset, chunk,
                                OCTREE_BALLS4_MPI_REAL, MPI_SUM,
                                OCTREE_BALLS4_MPI_ROOT, MPI_COMM_WORLD);
        else
            status = MPI_Reduce(values + offset, NULL, chunk,
                                OCTREE_BALLS4_MPI_REAL, MPI_SUM,
                                OCTREE_BALLS4_MPI_ROOT, MPI_COMM_WORLD);
        if (status != MPI_SUCCESS)
            return mpi_error(cmd, "MPI BALLS4 real reduction", status);
        offset += (size_t)chunk;
    }
    return SUCCESS;
}

int fcfc_octree_balls4_mpi_reduce_integers(struct cmdline_data *cmd,
                                           INTEGER *values, size_t count)
{
    ...
    while (offset < count) {
        ...
        if (mpi_rank == OCTREE_BALLS4_MPI_ROOT)
            status = MPI_Reduce(MPI_IN_PLACE, values + offset, chunk,
                                OCTREE_BALLS4_MPI_INTEGER, MPI_SUM,
                                OCTREE_BALLS4_MPI_ROOT, MPI_COMM_WORLD);
        else
            status = MPI_Reduce(values + offset, NULL, chunk,
                                OCTREE_BALLS4_MPI_INTEGER, MPI_SUM,
                                OCTREE_BALLS4_MPI_ROOT, MPI_COMM_WORLD);
        if (status != MPI_SUCCESS)
            return mpi_error(cmd, "MPI BALLS4 integer reduction", status);
        offset += (size_t)chunk;
    }
    return SUCCESS;
}

#endif /* OCTREEBALLS4MPI */
