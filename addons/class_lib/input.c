...
/*
    class_call(parser_read_string(pfc,"histZetaMFileName",&string1,&flag1,
                                  errmsg),
               errmsg,errmsg);
    if (flag1 == TRUE) {
        for (index=0;index<pfc->size;++index){
    ...
    class_call(parser_read_string(pfc,"mhistZetaFileName",&string1,&flag1,
                                  errmsg),
               errmsg,errmsg);
    if (flag1 == TRUE) {
        for (index=0;index<pfc->size;++index){
...
/*    if (flag1 == TRUE) {
        for (index=0;index<pfc->size;++index){
          if (strcmp(pfc->name[index],"columns") == 0){
              cmd->columns = strdup(pfc->value[index]);
            break;
          }
        }
    } */
