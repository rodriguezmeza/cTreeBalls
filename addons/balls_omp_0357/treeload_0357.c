...
global int MakeTree_0357(struct  cmdline_data* cmd,
                    struct  global_data* gd,
...
//B To debug cells:
    if (format_checked(cellsfilePath, sizeof(cellsfilePath),
                       "BALLS0357 cells path", "%s/cells%s.txt",
...
//E
    ...
    if (newtree(cmd, gd, ifile) == FAILURE)
        return FAILURE;
...
// By now it is only working with boxes centered at (0,0,...)
    ...
    if (FindRootCenter(cmd, gd, btab, nbody, ifile,
                       roottable[ifile]) == FAILURE)
        return FAILURE;
    if (centerBodies(btab, nbody, ifile, roottable[ifile]) == FAILURE)
        return FAILURE;
    if (FindRootCenter(cmd, gd, btab, nbody, ifile,
                       roottable[ifile]) == FAILURE)
        return FAILURE;
...
//E
    ...
    if (expandbox(cmd, gd, btab, nbody, ifile,
                  roottable[ifile]) == FAILURE)
        return FAILURE;
/*    if (cmd->verbose>=VERBOSEDEBUGINFO)
    verb_print(cmd->verbose,
...
#endif
        if (loadbody(cmd, gd, p, ifile) == FAILURE)
            return FAILURE;                          // Set body in a cell
        Nb(p) = 1;
        Radius(p) = 0.0;
#ifdef KappaAvgON
        KappaAvg(p) = Kappa(p);
...
/*    if (cmd->verbose>=VERBOSEDEBUGINFO)
    ...
    cpustartMiddle = CPUTIME;
    if (hackcellprop(cmd, gd, roottable[ifile], gd->rSizeTable[ifile],
                     0, ifile) == FAILURE)
        return FAILURE;
/*    if (cmd->verbose>=VERBOSEDEBUGINFO)
    verb_print(cmd->verbose,
...
//    }
    ...
    if (scanLevel(cmd, gd, ifile) == FAILURE)
        return FAILURE;                              // Scan pivot and root trees
//    if (cmd->verbose>=VERBOSEDEBUGINFO)
//    verb_print(cmd->verbose,
...
/*    if (cmd->verbose>=VERBOSEDEBUGINFO)
    ...
    if (smoothBodies(cmd, gd, btab, nbody) == FAILURE)
        return FAILURE;                              // Smooth cells
...
*/
        ...
        if (save_nodes(cmd, gd, ifile) == FAILURE)
            return FAILURE;
    } // ! scanLevel==0
...
// INMB*gd->nnodescanlev_rootTable[ifile]*sizeof(nodeptr),gd->nnodescanlev_rootTable[ifile]);
        ...
        if (cmd->verbose==3 &&
            save_nodes_root(cmd, gd, ifile) == FAILURE)
            return FAILURE;
    }
        ...
        if (Type(Subp(q)[qind]) == BODY || Type(Subp(q)[qind]) == BODY3) {
            ...
            if (dist2 == 0.0) { // look for these events in tmp/log file
                ...
                if (cballs_opt_no_check_equal_positions(cmd)) {
                    ...
                    cBALLS_FAIL(cmd,
                        "loadbody: two bodies have same position; consider "
                        "the option 'no-check-two-bodies-eq-pos'");
                }
            }
                       ...
                       cellptr p, real psize, int lev, int ifile)
{
    ...
    for (i = 0; i < NSUB; i++) {
        if ((q = Subp(p)[i]) != NULL) {
            subnhist[lev]++;
            if (Type(q) == CELL &&
                hackcellprop(cmd, gd, (cellptr) q, psize/2,
                             lev+1, ifile) == FAILURE)
                return FAILURE;
        ...
        if (cmpos[k] < Pos(p)[k] - psize/2 ||
            ...
            if (psize/2 > 2.710505e-20 + EPSILON) {
                cBALLS_FAIL(cmd,
                    ...
                    routineName, k, cmpos[k], Pos(p)[k] - psize/2,
                    Pos(p)[k] + psize/2, psize/2);
            } else {
...
#undef EPSILON
    ...
    if (Nb(p)>0) {
        ...
        cBALLS_FAIL(cmd, "%s: Nb = 0: %ld", routineName, Nb(p));
#endif
    }
...
local int save_nodes(struct  cmdline_data* cmd, struct  global_data* gd, int ifile)
{
    if (cmd->verbose==VERBOSEDEBUGINFO) {
        ...
        if(!(gd->outnodelev=fopen(gd->nodesfilePath, "w")))
            cBALLS_FAIL(cmd, "save_nodes: error opening file '%s'",
                        gd->nodesfilePath);
    }
//B
    if (!gd->flagSmoothCellMin && cballs_opt_smooth_min_cell(cmd) ) {
...
local int save_nodes_root(struct  cmdline_data* cmd, struct  global_data* gd, int ifile)
{
    ...
    if(!(gd->outnodelev=fopen(gd->nodesfilePath, "w")))
        cBALLS_FAIL(cmd, "save_nodes_root: error opening file '%s'",
                    gd->nodesfilePath);
