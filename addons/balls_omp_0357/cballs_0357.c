/*==============================================================================
 MODULE: cballs_0357.c				[cTreeBalls]
...
local int EvalHist_0357(struct  cmdline_data* cmd, struct  global_data* gd);
 ...
 */
int MainLoop_0357(struct  cmdline_data* cmd, struct  global_data* gd)
{
    string routineName = "MainLoop_0357";
    bodyptr p,q;
    real kavg;
    INTEGER in;
...
//                  "\n\t%s: smooth cell min: try making tree...\n\n", routineName);
        ...
        DO_BODY(p,bodytable[gd->iCatalogs[0]],
            ...
            Update(p) = TRUE;
        if (MakeTree_0357(cmd, gd, bodytable[gd->iCatalogs[0]],
                         gd->nbodyTable[gd->iCatalogs[0]],
                         gd->iCatalogs[0]) == FAILURE)
            return FAILURE;
//B
...
/*        verb_print(cmd->verbose,
    ...
    if (scanopt(cmd->options, "smooth")
        ...
        DO_BODY(p, bodytable[ifile], bodytable[ifile]+cmd->nbody)
            Update(p) = TRUE;
        if (MakeTree_0357(cmd, gd, bodytable[ifile], cmd->nbody, 0) == FAILURE)
            return FAILURE;
//B
        INTEGER new_nbody = gd->nbodysm;
        bodyptr replacement = NULL;
        if (cballs_malloc_checked((void **)&replacement, (size_t)new_nbody,
                                  sizeof(*replacement),
                                  "BALLS0357 smoothed body table",
                                  cmd->error_message,
...
//E
    ...
    if ( scanopt(cmd->options, "smooth")
        ...
        if (MakeTree_0357(cmd, gd, bodytable[ifile], cmd->nbody, 0) == FAILURE)
            return FAILURE;
//B
        INTEGER new_nbody = gd->nbodySel;
        bodyptr replacement = NULL;
        if (cballs_malloc_checked((void **)&replacement, (size_t)new_nbody,
                                  sizeof(*replacement),
                                  "BALLS0357 selected body table",
                                  cmd->error_message,
                                  _ERRORMSGSIZE_) == FAILURE)
            return FAILURE;
        free(bodytable[ifile]);
...
//    verb_print(cmd->verbose, "\n\tMainLoop: make-tree: try making tree...\n\n");
    ...
    if (scanopt(cmd->options, "stop")) {
        if (!strnull(cmd->outfile) &&
            OutputData(cmd, gd, bodytable, gd->nbodyTable, ifile) == FAILURE)
            return FAILURE;
//        verb_print(cmd->verbose, "\n\tMainLoop: stopping...\n\n");
        verb_print_min_info(cmd->verbose, cmd->verbose_log, gd->outlog,
                               "\n\t%s: stopping...\n\n", routineName);
    ...
    if (EvalHist_0357(cmd, gd) == FAILURE)
        return FAILURE;
...
local int EvalHist_0357(struct  cmdline_data* cmd, struct  global_data* gd)
{
    ...
    if (correlation_string_to_int(cmd, gd, &correlation_int) == FAILURE)
        return FAILURE;

//    verb_print(cmd->verbose,
//               "\n\tevalHist: with balls tree-omp method\n\n");
//    MainLoop_0357(cmd, gd);
    for (ifile=0; ifile<gd->ninfiles; ifile++) {
        DO_BODY(p,bodytable[ifile],bodytable[ifile]+gd->nbodyTable[ifile])
        Update(p) = TRUE;
        if (MakeTree_0357(cmd, gd, bodytable[ifile],
                         gd->nbodyTable[ifile], ifile) == FAILURE)
            return FAILURE;
    }
    if (searchcalc_balls_omp_0357(cmd, gd, bodytable, gd->nbodyTable,
                                 1, gd->nbodyTable,
                                 gd->iCatalogs[0],
                                 gd->iCatalogs[1]) == FAILURE)
        return FAILURE;
/*
...
local int PrintHistNN(struct  cmdline_data* cmd, struct  global_data* gd)
{
    ...
    if (scanopt(cmd->options, "and-CF") && PrintHistCF(cmd, gd) == FAILURE)
        return FAILURE;
