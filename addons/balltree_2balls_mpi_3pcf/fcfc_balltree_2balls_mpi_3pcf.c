/* Deterministic MPI runtime for the FCFC ball-tree LogMultipole estimator. */
...
#define BALLTREE_2BALLS_MPI_3PCF_ROOT 0
#define BALLTREE_2BALLS_MPI_3PCF_METHOD "balltree-2balls-mpi_3pcf"

#if defined(SINGLEPREC)
#define BALLTREE_2BALLS_MPI_3PCF_REAL MPI_FLOAT
#else
#define BALLTREE_2BALLS_MPI_3PCF_REAL MPI_DOUBLE
#endif

#ifdef LONGINT
#define BALLTREE_2BALLS_MPI_3PCF_INTEGER MPI_LONG
#else
#define BALLTREE_2BALLS_MPI_3PCF_INTEGER MPI_INT
#endif
...
static int method_selected(const struct cmdline_data *cmd)
{
    return cmd->searchMethod != NULL
        && strcmp(cmd->searchMethod, BALLTREE_2BALLS_MPI_3PCF_METHOD) == 0;
}
...
int fcfc_balltree_2balls_mpi_3pcf_prepare(struct cmdline_data *cmd,
                                          struct global_data *gd)
{
    ...
    if (mpi_active) {
        if (mpi_rank != BALLTREE_2BALLS_MPI_3PCF_ROOT) {
            cmd->verbose = 0;
            cmd->verbose_log = 0;
            gd->flagPrint = FALSE;
        }
    ...
    if (finalized) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 ...
                 BALLTREE_2BALLS_MPI_3PCF_METHOD);
        return FAILURE;
    }
...
int fcfc_balltree_2balls_mpi_3pcf_finalize(struct cmdline_data *cmd)
{
    int finalized = FALSE;
    int status;
...
int fcfc_balltree_2balls_mpi_3pcf_active(void) { return mpi_active; }
int fcfc_balltree_2balls_mpi_3pcf_is_root(void)
{
    return !mpi_active || mpi_rank == BALLTREE_2BALLS_MPI_3PCF_ROOT;
}
int fcfc_balltree_2balls_mpi_3pcf_rank(void) { return mpi_rank; }
int fcfc_balltree_2balls_mpi_3pcf_size(void) { return mpi_size; }

int fcfc_balltree_2balls_mpi_3pcf_output_enabled(
        struct cmdline_data *cmd)
{
    return !method_selected(cmd) || !mpi_active
        || mpi_rank == BALLTREE_2BALLS_MPI_3PCF_ROOT;
}

int fcfc_balltree_2balls_mpi_3pcf_consensus(
...
int fcfc_balltree_2balls_mpi_3pcf_task_owned(INTEGER task)
{
    if (!mpi_active || mpi_size <= 1) return TRUE;
    return (int)(task % (INTEGER)mpi_size) == mpi_rank;
}

int fcfc_balltree_2balls_mpi_3pcf_reduce_reals(
    ...
    while (offset < count) {
        ...
        if (mpi_rank == BALLTREE_2BALLS_MPI_3PCF_ROOT)
            status = MPI_Reduce(MPI_IN_PLACE, values + offset, chunk,
                                BALLTREE_2BALLS_MPI_3PCF_REAL, MPI_SUM,
                                BALLTREE_2BALLS_MPI_3PCF_ROOT,
                                MPI_COMM_WORLD);
        else
            status = MPI_Reduce(values + offset, NULL, chunk,
                                ...
                                MPI_COMM_WORLD);
        if (status != MPI_SUCCESS)
            return mpi_error(cmd, "MPI LogMultipole real reduction", status);
        offset += (size_t)chunk;
    }
    return SUCCESS;
}

int fcfc_balltree_2balls_mpi_3pcf_reduce_integers(
        struct cmdline_data *cmd, INTEGER *values, size_t count)
{
    ...
    while (offset < count) {
        ...
        if (mpi_rank == BALLTREE_2BALLS_MPI_3PCF_ROOT)
            status = MPI_Reduce(MPI_IN_PLACE, values + offset, chunk,
                                BALLTREE_2BALLS_MPI_3PCF_INTEGER, MPI_SUM,
                                BALLTREE_2BALLS_MPI_3PCF_ROOT,
                                MPI_COMM_WORLD);
        else
            status = MPI_Reduce(values + offset, NULL, chunk,
                                ...
                                MPI_COMM_WORLD);
        if (status != MPI_SUCCESS)
            return mpi_error(cmd, "MPI LogMultipole integer reduction",
                             status);
        offset += (size_t)chunk;
    }
    return SUCCESS;
