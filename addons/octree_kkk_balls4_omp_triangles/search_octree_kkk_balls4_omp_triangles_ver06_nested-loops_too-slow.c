...
#else
#error needs `BALLS`, switch it on in addons/Makefile_addons_settings
#endif
...
// if only sum bodies, it is too slow, CPU 110m
//#pragma omp for nowait schedule(dynamic) collapse(2) // slower
...
// if only sum bodies-cell, it is too slow, CPU 114m
local void sumcell_balls6_omp(struct cmdline_data* cmd,
                                        struct  global_data* gd,
                              cellptr start, cellptr finish, bodyptr p0,
                              gdhistptr_sincos_omp_kkk hist)
...
//B second vertix h
                        hb = ((nodeptr) h);
                        if (accept_body(cmd, gd, (bodyptr)p0, hb, &dr1_h, dr_h)) {
                            if(dr1_h>cmd->rminHist) {
                                ibodycount++;
                                if (cmd->rminHist==0)
                                    l = (int)(cmd->logHistBinsPD*(rlog10(dr1_h)
                                ...
                                // needs to multiply xi by Weight(p) of the cell q
                                    xih = Kappa(hb);
                            //B Component of hb with respect to the axis of ref.
                                    DOTVP(s, dr_h, hist->dr0);
                                    CROSSVP(pr0,hist->dr0,Pos(h));
                                    DOTVP(sy, dr_h, pr0);
                                    theta2 = angle_dxdy(s, sy);
