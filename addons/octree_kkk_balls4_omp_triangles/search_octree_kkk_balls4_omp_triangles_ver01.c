...
#ifdef TRIANGLESTC
local void sumnode_balls6_omp_triangles(struct cmdline_data* cmd,
                                        struct  global_data* gd,
                              cellptr start, cellptr finish, bodyptr p0,
                                        gdhistptr_sincos_omp_kkk hist);
local void sumcell_balls6_omp_triangles(struct cmdline_data* cmd,
                                        struct  global_data* gd,
                              cellptr, cellptr, bodyptr,
                              gdhistptr_sincos_omp_kkk);
local void sumcellcell_balls6_omp_triangles(struct cmdline_data* cmd,
                                  struct  global_data* gd,
                                  cellptr, cellptr, nodeptr,
                                  gdhistptr_sincos_omp_kkk);
#endif
...
#pragma omp parallel default(none)                                          \
    shared(cmd,gd,btable,nbody,roottable,                                   \
           ...
           ipmin,ipmax,cat1,cat2,nodetablescanlev,nodetablescanlev_root, gdl, gdlN)
#else
#pragma omp parallel default(none)                                          \
    shared(cmd,gd,btable,nbody,roottable,                                   \
           ipfalse,icountNbRmin,icountNbRminOverlap,                        \
...
#endif
        verb_print(cmd->verbose,
                   "octree-kkk-balls4-omp: p falses found = %ld and %e %e %e\n",
                   ipfalse, num, den, xi);
        for (mm=1; mm<=cmd->mChebyshev+1; mm++) {
                    ...
                    if ( ((Nb(*ap)<=gd->nsmooth[0])
                        ...
                        Nb(bptr) = Nb(*ap);
                         ...
                         if (nodes_condition_balls(cmd, gd, p, *ap, &dr1, dr)) {
                             if (!cballs_opt_no_two_balls(cmd)
                                 && Type(p) == CELL ) {
                                 sumcellcell_balls6_omp_triangles(cmd, gd,
                                        (cellptr)(*ap),
...
//#else
//        verb_print_debug(1, "\nAqui voy (2.2)\n");
        sumcell_balls6_omp_triangles(cmd, gd, histb->interact, cptr, (bodyptr) p,
                           &hist1sincos);
//        verb_print_debug(1, "\nAqui voy (2.3)\n");
//#endif
    }
//#ifdef TRIANGLESTC
    sumnode_balls6_omp_triangles(cmd, gd, bptr, histb->interact + histb->actlen,
...
//    if (hist.drpq2==0) continue;
    ...
    if (cballs_opt_smooth_pivot(cmd)) {
        ...
        for (n = 1; n <= cmd->sizeHistN; n++) {
            hist1sincos_N.histNNSubthread[n] =
                          ((real)NbRmin(p))*hist1sincos_N.histNNSubthread[n];
        }
    }
    computeBodyProperties_sincos_kkk_sum_balls6_omp(cmd, gd,
                                    p, nbody, histsincos, &hist1sincos);
                              ...
                              gdhistptr_sincos_omp_kkk hist)
{
    ...
    for (p = start; p < finish-1; p++) {
#else
    for (p = start; p < finish; p++) {
#endif
        pb = ((nodeptr) p);
//        verb_print_debug(1, "Aqui voy (2.2.1): body %ld\n", start-p);
        if (accept_body(cmd, gd, p0, pb, &dr1, dr)) {
            //B smooth-pivot
            if (cballs_opt_smooth_pivot(cmd)) {
                if (dr1<=gd->rsmooth[0]) {
...
//                                    hist->histNNSubthread[l]
//                                        = hist->histNNSubthread[l] + 1.;
                                    xih = Kappa(hb);
                ...
                //E
//                    theta = rabs(theta2-theta1); // squezed triangle?
