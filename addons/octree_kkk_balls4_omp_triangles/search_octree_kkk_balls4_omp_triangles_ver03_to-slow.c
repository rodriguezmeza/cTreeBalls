...
typedef struct {
    ...
    real ***xiOUTVPsinTC;
    real **histNNSubthreadTC;
} gdhist_sincos_omp_kkk_red, *gdhistptr_sincos_omp_kkk_red;
//E Define structures:
...
local int search_init_sincos_omp_kkk_red(struct cmdline_data* cmd,
                                         struct global_data* gd,
                                         gdhistptr_sincos_omp_kkk_red hist);
local int search_free_sincos_omp_kkk_red(struct cmdline_data* cmd,
                                     struct global_data* gd,
                                     gdhistptr_sincos_omp_kkk_red hist);
...
#pragma omp parallel default(none)                                          \
    shared(cmd, gd, start, finish, p0, hist)
    {
        cellptr p;
        vector dr;
        real dr1;
        ...
        gdhist_sincos_omp_kkk_red histr;
        search_init_sincos_omp_kkk_red(cmd, gd, &histr);
                    ...
                    if (n<=cmd->sizeHistN && n>=1) {
                        ...
                        for (h = start; h < finish; h++) {
                            ...
                            if (accept_body(cmd, gd, p0, hb, &dr1_h, dr_h)) {
                                if(dr1_h>cmd->rminHist) {
                                    ...
                                    if (cmd->rminHist==0)
                                        l =
                                        (int)(cmd->logHistBinsPD*(rlog10(dr1_h)
                        ...
                        //E
                                        ...
                                        for (int m=1; m<=cmd->mChebyshev+1; m++) {
                                            histr.xiOUTVPcosTC[m][n][l]
                                            += histr.histXithreadcosTC[m];
                                            histr.xiOUTVPsinTC[m][n][l]
                                            += histr.histXithreadsinTC[m];
                                        }
                                        histr.histNNSubthreadTC[n][l] =
                                        histr.histNNSubthreadTC[n][l] + 1.;
                                    } // ! 1 < l < HistN
                                } // ! dr1_h > rmin
                            } // ! accept_body h
                        } // ! loop h
...
#pragma omp critical
    ...
    search_free_sincos_omp_kkk_red(cmd, gd, &histr);     // free memory
    } // end pragma omp parallel
}
