...
/*
 ...
 Return:
    void
 */
global int checkstop(struct cmdline_data* cmd,
                      struct  global_data* gd)
{
    string routineName = "checkstop";
    char   buf[200];
    FILE *fd;
    double cpudt;
 ...
 */
global int savestate(struct cmdline_data* cmd,
                      struct  global_data* gd,
                      string pattern)
{
...
//B cmdline_data
    ...
    //B new
    safewrite(&cmd->nsmooth, sizeof(int), str);
    //E
    ...
    nchars = strlen(cmd->preScript) + 1;
    safewrite(&nchars, sizeof(int), str);
    safewrite(cmd->preScript, nchars * sizeof(char), str);
    nchars = strlen(cmd->posScript) + 1;
    safewrite(&nchars, sizeof(int), str);
    safewrite(cmd->posScript, nchars * sizeof(char), str);
...
//  here it is the necessary lines...
    ...
    safewrite(&cmd->scanLevelMin, sizeof(int), str);
    //E
#endif
#ifdef IOLIB
...
//B cmdline_data
    ...
    //B new
    saferead(&cmd->nsmooth, sizeof(int), str);
    //E
    ...
    //B Miscellaneous parameters
    saferead(&nchars, sizeof(int), str);
    cmd->preScript = (string) allocate(nchars * sizeof(char));
    saferead(cmd->preScript, nchars * sizeof(char), str);
    saferead(&nchars, sizeof(int), str);
    cmd->posScript = (string) allocate(nchars * sizeof(char));
    saferead(cmd->posScript, nchars * sizeof(char), str);
...
//  here it is the necessary lines...
    ...
    saferead(&cmd->scanLevelMin, sizeof(int), str);
#endif
    //E
#endif
#ifdef IOLIB
...
//    gsl_matrix_complex *NhistXi_gsl;
    ...
    for (ifile=0; ifile<gd->ninfiles; ifile++) {
        //B
        saferead(&nchars, sizeof(int), str);
        if (nchars <= 0 || nchars > (int)sizeof(gd->infilenames[ifile])) {
            snprintf(cmd->error_message, _ERRORMSGSIZE_,
                "restorestate: bad infilenames[%d] length %d", ifile, nchars);
            fclose(str);
            return FAILURE;
        }
    ...
    for (ifile=0; ifile<gd->ninfiles; ifile++) {
        //B
        saferead(&nchars, sizeof(int), str);
        if (nchars <= 0 || nchars > (int)sizeof(gd->infilefmtname[ifile])) {
            snprintf(cmd->error_message, _ERRORMSGSIZE_,
                     "restorestate: bad infilefmtname[%d] length %d", ifile, nchars);
            fclose(str);
            return FAILURE;
        }
...
//  in the same order as was given in savestate routine
    ...
    if (ndefault == 0) {
        if (format_checked(buf, sizeof(buf),
            "buf", "%s/%s%s",cmd->rootDir,fname,"-restorevalues") != 0)
            return FAILURE;
        ...
        verb_print_q(3,cmd->verbose,
                    ...
                    ndefault, ipos, dp);
        if (format_checked(buf, sizeof(buf),
            "buf", "%s/%s%s",cmd->rootDir,dp,"-restorevalues") != 0)
            return FAILURE;
    ...
    if(!(fdout=fopen(buf,"w"))) {
        ...
        if (cmd->verbose>=VERBOSEDEBUGINFO)
            verb_print(cmd->verbose,
                       "\nPrintParamterFile: preScript: %s\n", cmd->preScript);
        if (cmd->verbose>=VERBOSEDEBUGINFO)
            verb_print(cmd->verbose,
                       "\nPrintParamterFile: posScript: %s\n", cmd->posScript);
