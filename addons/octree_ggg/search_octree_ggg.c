/* ==============================================================================
 MODULE: search_octree_ggg.c		[cTreeBalls]
 Written by: M.A. Rodriguez-Meza
 Starting date:    april 2023
 Purpose: 3-point correlation function computation
 Language: C
 Use: searchcalc_octree_ggg(cmd, gd, btable, nbody,
...
} gdl_sincos_ggg, *gdlptr_sincos_ggg;
...
typedef struct {
    ...
    real cosb;
    real sinb;
} gdhist_sincos_ggg, *gdhistptr_sincos_ggg;
...
typedef struct {
 ...
 #ifdef THREEPCFSHEAR
 #endif
} gdl_sincos_ggg_N, *gdlptr_sincos_ggg_N;
...
typedef struct {
    ...
    real cosb;
    real sinb;
} gdhist_sincos_ggg_N, *gdhistptr_sincos_ggg_N;
...
local void sumnode_sincos_cell(struct  cmdline_data* cmd,
                               ...
                               gdhistptr_sincos_ggg hist);

local int search_init_gd_sincos_ggg(struct  cmdline_data* cmd,
                                        struct  global_data* gd,
                                        gdlptr_sincos_ggg);
local int search_free_gd_sincos_ggg(struct  cmdline_data* cmd,
                                         struct  global_data* gd,
                                        gdlptr_sincos_ggg);
local int search_init_sincos_ggg(struct  cmdline_data* cmd,
                                  struct  global_data* gd,
                                     gdhistptr_sincos_ggg hist);
local int search_free_sincos_ggg(struct  cmdline_data* cmd,
                                         struct  global_data* gd,
                                     gdhistptr_sincos_ggg hist);
...
local void sumnode_sincos_cell_N(struct  cmdline_data*,
                                 ...
                                 gdhistptr_sincos_ggg,
                                 gdhistptr_sincos_ggg_N);
local int search_init_gd_sincos_ggg_N(struct  cmdline_data* cmd,
                                          struct  global_data* gd,
                                          gdlptr_sincos_ggg_N);
local int search_free_gd_sincos_ggg_N(struct  cmdline_data* cmd,
                                         struct  global_data* gd,
                                          gdlptr_sincos_ggg_N);
local int search_init_sincos_ggg_N(struct  cmdline_data* cmd,
                                  struct  global_data* gd,
                                     gdhistptr_sincos_ggg_N hist);
local int search_free_sincos_ggg_N(struct  cmdline_data* cmd,
                                         struct  global_data* gd,
                                     gdhistptr_sincos_ggg_N);
...
local int polarix_init(struct  cmdline_data* cmd,
                               struct  global_data* gd,
                               bodyptr p, gdhistptr_sincos_ggg hist);
 ...
 */
global int searchcalc_octree_ggg(struct cmdline_data* cmd,
                                     struct  global_data* gd,
                                     bodyptr *btable, INTEGER *nbody,
                                     INTEGER ipmin, INTEGER *ipmax,
                                     int cat1, int cat2)
{
    string routineName = "searchcalc_octree_ggg";
    double cpustart;
    gdl_sincos_ggg gdl;
#ifdef NMultipoles
    gdl_sincos_ggg_N gdlN;
#endif
    ...
    if (cmd->useLogHist==FALSE &&
        (strcmp(cmd->searchMethod,"octree-ggg") == 0))
    verb_print_normal_info(cmd->verbose, cmd->verbose_log, gd->outlog,
            "%s: can´t have loghist false and octree-ggg (%d %s)\n",
...
#error `OPENMPMACHINE` is not defined. Switch it on in Makefile_settings
    ...
    search_init_gd_sincos_ggg(cmd, gd, &gdl);
#ifdef NMultipoles
    search_init_gd_sincos_ggg_N(cmd, gd, &gdlN);
#endif
...
//      if (main_thread_id == current_thread_id)
    ...
    //B init:
    gdhist_sincos_ggg hist;
    search_init_sincos_ggg(cmd, gd, &hist);
#ifdef NMultipoles
    gdhist_sincos_ggg_N histN;
    search_init_sincos_ggg_N(cmd, gd, &histN);
#endif
//      if (main_thread_id == current_thread_id)
          verb_print_normal_info(cmd->verbose, cmd->verbose_log, gd->outlog,
...
#ifdef NMultipoles
    search_free_sincos_ggg_N(cmd, gd, &histN);  // free memory
#endif
    search_free_sincos_ggg(cmd, gd, &hist);     // free memory
...
#ifdef TWOPCF
    if (cballs_opt_compute_histn(cmd)) PrintHistNN(cmd, gd, &gdl);
...
//B free memory
#ifdef NMultipoles
    search_free_gd_sincos_ggg_N(cmd, gd, &gdlN);
#endif
    search_free_gd_sincos_ggg(cmd, gd, &gdl);
//E
                                  ...
                                  struct  global_data* gd)
{
    ...
    if (cmd->usePeriodic==TRUE)
        verb_print_normal_info(cmd->verbose, cmd->verbose_log, gd->outlog,
    "%s: warning!! can´t have periodic boundaries and OCTREEGGG definition (usePeriodic=%d)\nSet usePeriodic=false\n",
                            routineName,cmd->usePeriodic);

    if (cmd->useLogHist==FALSE)
    verb_print_normal_info(cmd->verbose, cmd->verbose_log, gd->outlog,
"%s: warning!! can´t have normal scale hist and OCTREEGGG definition (useLogHist=%d)\nSet useLogHist=true\n",
...
#if NDIM == 2
//error("CheckParameters: OCTREEGGG definition works only in a 3D unit sphere");
    verb_print_min_info(cmd->verbose, cmd->verbose_log, gd->outlog,
            "OCTREEGGG definition working in a 2D box... \n");
#endif
...
local int PrintHistZetaG(struct  cmdline_data* cmd,
                        struct  global_data* gd, gdlptr_sincos_ggg gdl)
{
    int n1, n2, l, m;
    stream outstr;
    char namebuf[256];
