/* ==============================================================================
 MODULE: search_octree_kkk_balls4_omp.c		[cTreeBalls]
...
/*
 ...
 To be called using: search=octree-kkk-balls4-omp
...
global int searchcalc_octree_kkk_balls4_omp(struct cmdline_data* cmd,
                                             ...
                                             int cat1, int cat2)
{
    string routine_name = "searchcalc_octree_kkk_balls4_omp";
    double cpustart;
                                  ...
                                  struct  global_data* gd)
{
    ...
    if (cmd->usePeriodic==TRUE)
        cBALLS_FAIL(cmd,
            ...
            "OCTREEKKKBALLS4OMP (usePeriodic=%d); set usePeriodic=false\n",
            cmd->usePeriodic);
    if (cmd->useLogHist==FALSE)
        cBALLS_FAIL(cmd,
            "CheckParameters: can't have normal-scale histograms and "
            "OCTREEKKKBALLS4OMP (useLogHist=%d); set useLogHist=true\n",
            cmd->useLogHist);
#if !defined(THREEPCFCONVERGENCE) && !defined(TWOPCF)
    cBALLS_FAIL(cmd,
        "CheckParameters: OCTREEKKKBALLS4OMP requires TWOPCF or TPCF support\n");
#endif
#if NDIM == 2
    cBALLS_FAIL(cmd,
        "CheckParameters: OCTREEKKKBALLS4OMP works only in 3D\n");
#endif
#ifdef NMultipoles
