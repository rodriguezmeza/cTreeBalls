                                       ...
                                       gdlptr_sincos_omp_ggg_N gdlN)
{
    ...
    for (n1=1; n1<=cmd->sizeHistN; n1++) {
        for (n2=1; n2<=cmd->sizeHistN; n2++) {
            ggg_complex nzero = ggg_histogram_mode(
                ...
                0, n1, n2);
            const size_t bin = (size_t)(n1-1)*cmd->sizeHistN + n2-1;
            real smallest_pivot = DBL_MAX, largest_pivot = 0.0;
            gd->scalar_window_w0[bin] = nzero.re;
            gd->scalar_window_status[bin] = CBALLS_WINDOW_NONFINITE;
            real tolerance;
            ...
            for (row=1; row<=nmax+1; row++) {
                gdl->histZetaM_EE[row][n1][n2] = NAN;
                gdl->histZetaM_EE_Im[row][n1][n2] = NAN;
            }
            ...
            if (nzero.re <= 0.0 || ggg_complex_abs2(nzero) <= DBL_MIN) {
                gd->scalar_window_status[bin] = CBALLS_WINDOW_EMPTY;
                singular_bins++;
                continue;
            }
            ...
            gd->scalar_window_status[bin] = CBALLS_WINDOW_SINGULAR;
            ...
            for (column=0; column<multipoles; column++) {
                ...
                if (pivot_norm <= tolerance*tolerance) {
                    gd->scalar_window_pivot_ratio[bin] = 0.0;
                    singular = TRUE;
                    break;
                }
                smallest_pivot = fmin(smallest_pivot, sqrt(pivot_norm));
                largest_pivot = fmax(largest_pivot, sqrt(pivot_norm));
                if (pivot != column) {
                    int k;
            ...
            gd->scalar_window_status[bin] = CBALLS_WINDOW_VALID;
            gd->scalar_window_pivot_ratio[bin] = smallest_pivot/largest_pivot;
            for (row=nmax; row<multipoles; row++) {
    ...
    if (singular_bins > 0)
        verb_print_normal_info(
            ...
            "were unsupported and set to NaN\n", singular_bins);
    free(matrix);
    free(rhs);
    if (gd->rootDirFlag && cballs_scalar_window_write(cmd, gd) == FAILURE)
        return FAILURE;
...
#ifdef SMOOTHPIVOT
        if (search_compute_HistN_ggg(cmd, gd, nbody[cat1]-ipfalse, &gdl) == FAILURE)
            goto publication_failed;
#else
        if (search_compute_HistN_ggg(cmd, gd, nbody[cat1], &gdl) == FAILURE)
            goto publication_failed;
#endif
    }
#endif // ! TWOPCF
...
#define GGG_PUBLISH_OR_FAIL(call) \
    do { if ((call) == FAILURE) goto publication_failed; } while (0)
#ifdef TWOPCF
    if (cballs_opt_compute_histn(cmd)) GGG_PUBLISH_OR_FAIL(PrintHistNN(cmd, gd, &gdl));
    GGG_PUBLISH_OR_FAIL(PrintHistXi2pcf(cmd, gd, &gdl));
#endif

    GGG_PUBLISH_OR_FAIL(PrintHistrBins(cmd, gd));
    ...
    if (run_threepcf) {
        ...
        if (cballs_opt_no_normalize_histzeta(cmd))
            GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos(cmd, gd, &gdl));
        else
            GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos_normalized(cmd, gd, &gdl, &gdlN));
#else
        GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos(cmd, gd, &gdl));
#endif // ! NONORMHIST

          if (need_histn_multipoles)
              GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos_N(cmd, gd, &gdlN));
    ...
    GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos(cmd, gd, &gdl));
...
#ifdef NONORMHIST
            if (cballs_opt_no_normalize_histzeta(cmd))
                GGG_PUBLISH_OR_FAIL(PrintHistZetaMm_sincos(cmd, gd, &gdl));
            else
                GGG_PUBLISH_OR_FAIL(PrintHistZetaMm_sincos_normalized(cmd, gd, &gdl, &gdlN));
#else
            GGG_PUBLISH_OR_FAIL(PrintHistZetaMm_sincos(cmd, gd, &gdl));
#endif // ! NONORMHIST

            if (need_histn_multipoles)
                GGG_PUBLISH_OR_FAIL(PrintHistZetaMm_sincos_N(cmd, gd, &gdlN));
        ...
        if (cballs_opt_out_histzetag(cmd)) {
            GGG_PUBLISH_OR_FAIL(PrintHistZetaGm_sincos(cmd, gd, &gdl));
            GGG_PUBLISH_OR_FAIL(PrintHistZetaG(cmd, gd, &gdl));
            GGG_PUBLISH_OR_FAIL(PrintHistZetaMZetaGm_sincos(cmd, gd, &gdl));
        }
    ...
    //  make this two questions more precise...
        if (cballs_opt_no_normalize_histzeta(cmd)) {
            if (cballs_opt_edge_corrections(cmd)) {
                GGG_PUBLISH_OR_FAIL(PrintHistZetaM_sincos_edge_effects(cmd, gd, &gdl, &gdlN));
            }
        }
#endif // ! NONORMHIST
#endif // ! NMultipoles
...
// Share the ordered-to-unordered count and N^2 estimator contract with core.
local int search_compute_HistN_ggg(struct cmdline_data *cmd,
                                  struct global_data *gd, INTEGER nbody,
                                  gdlptr_sincos_omp_ggg gdl)
{
    return search_normalize_count_histograms(cmd, gd, nbody, gdl->histNN, gdl->histCF);
}
#endif // ! TWOPCF
...
"%s: warning!! can´t have normal scale hist and OCTREEGGGOMP definition (useLogHist=%d)\nSet useLogHist=true\n",
    ...
    verb_print_min_info(cmd->verbose, cmd->verbose_log, gd->outlog,
                        "radial bins: %s\n",
                        cmd->useLogHist ? "logarithmic" : "linear");
