...
#ifndef SHEAR_MPI_HELPER_HEADER
#define SHEAR_MPI_HELPER_HEADER "fcfc_octree_shear_sphere_2balls_mpi.h"
#define SHEAR_MPI_REDUCE_REALS fcfc_octree_shear_sphere_2balls_mpi_reduce_reals
#define SHEAR_MPI_REDUCE_INTEGERS fcfc_octree_shear_sphere_2balls_mpi_reduce_integers
#define SHEAR_MPI_TASK_OWNED fcfc_octree_shear_sphere_2balls_mpi_task_owned
#define SHEAR_MPI_IS_ROOT fcfc_octree_shear_sphere_2balls_mpi_is_root
#define SHEAR_MPI_CONSENSUS fcfc_octree_shear_sphere_2balls_mpi_consensus
#define SHEAR_MPI_RANK fcfc_octree_shear_sphere_2balls_mpi_rank
#define SHEAR_MPI_SIZE fcfc_octree_shear_sphere_2balls_mpi_size
#endif
#include SHEAR_MPI_HELPER_HEADER
#if !defined(SHEAR_MPI_REDUCE_REALS) \
    || !defined(SHEAR_MPI_REDUCE_INTEGERS) \
    || !defined(SHEAR_MPI_TASK_OWNED) \
    || !defined(SHEAR_MPI_IS_ROOT) \
    || !defined(SHEAR_MPI_CONSENSUS) \
    || !defined(SHEAR_MPI_RANK) \
    || !defined(SHEAR_MPI_SIZE)
#error "SHEAR_MPI_ENABLED requires SHEAR_MPI helper macros"
#endif
#endif
#ifdef BALLS4SCANLEV
...
typedef real shear_vector2 __attribute__((vector_size(2*sizeof(real))));
#define SHEAR_HAVE_VECTOR2 1
#endif
...
typedef struct {
    ...
    uint64_t pivots;
    shear_profile_sample radial;
    shear_profile_sample transport;
    shear_profile_sample ring;
} shear_profile_counters;
...
static void shear_accumulate_complex(shear_complex *target,
                                     shear_complex increment)
{
#ifdef SHEAR_HAVE_VECTOR2
    shear_vector2 value;
    const shear_vector2 addend = {increment.re, increment.im};

    memcpy(&value, target, sizeof(value));
    value += addend;
    memcpy(target, &value, sizeof(value));
#else
    target->re += increment.re;
    target->im += increment.im;
#endif
}

static void shear_accumulate_complex_product(shear_complex *target,
                                             shear_complex left,
                                             shear_complex right)
{
#ifdef SHEAR_HAVE_VECTOR2
    shear_vector2 value;
    const shear_vector2 right_vector = {right.re, right.im};
    const shear_vector2 rotated_right = {-right.im, right.re};

    memcpy(&value, target, sizeof(value));
    value += right_vector*left.re + rotated_right*left.im;
    memcpy(target, &value, sizeof(value));
#else
    shear_accumulate_complex(target, shear_mul(left, right));
#endif
}

static void shear_accumulate_complex_scaled(shear_complex *target,
                                            shear_complex value,
                                            real scale)
{
#ifdef SHEAR_HAVE_VECTOR2
    shear_vector2 total;
    const shear_vector2 increment = {value.re, value.im};

    memcpy(&total, target, sizeof(total));
    total += increment*scale;
    memcpy(target, &total, sizeof(total));
#else
    shear_accumulate_complex(target, shear_scale(value, scale));
#endif
}
...
static double shear_profile_wall_time(void)
{
#ifdef OPENMPCODE
...
static bool shear_profile_sample_begin(shear_profile_sample *sample,
                                       double *started)
{
    if (sample == NULL) return FALSE;
    sample->calls++;
    if ((sample->calls - 1) % 256 != 0) return FALSE;
    *started = shear_profile_wall_time();
    return TRUE;
}

static void shear_profile_sample_end(shear_profile_sample *sample,
                                     double started, bool active)
{
    if (!active) return;
    sample->sampled_seconds += shear_profile_wall_time() - started;
    sample->samples++;
}

static double shear_profile_estimate(const shear_profile_sample *sample)
{
    return sample->samples > 0
        ? sample->sampled_seconds*(double)sample->calls/(double)sample->samples
        : 0.0;
}
...
{
    (void)bins;
    return (size_t)bin*(size_t)(2*ring_max + 1)
        + (size_t)(order + ring_max);
}

static size_t shear_weight_ring_index(int order, int bin, int ring_max)
{
    const int absolute_order = order < 0 ? -order : order;
    return (size_t)bin*(size_t)(ring_max + 1) + (size_t)absolute_order;
}

static shear_complex shear_weight_ring_value(const shear_complex *ring,
                                              int order, int bin,
                                              int ring_max)
{
    shear_complex value = ring[
        shear_weight_ring_index(order, bin, ring_max)];
    return order < 0 ? shear_conj(value) : value;
}
...
static int shear_radial_bin_profiled(const shear_pivot_workspace *work,
                                     real distance)
{
    shear_profile_sample *sample;
    double started = 0.0;
    bool active;
    int bin;

    if (work->profile == NULL)
        return shear_radial_bin(work->cmd, work->gd, distance);
    sample = &work->profile->radial;
    active = shear_profile_sample_begin(sample, &started);
    bin = shear_radial_bin(work->cmd, work->gd, distance);

    shear_profile_sample_end(sample, started, active);
    return bin;
}

static bool shear_interval_within_radial_bin(
        const struct cmdline_data *cmd, const struct global_data *gd,
        real lower, real upper, int bin)
{
    real lower_edge;
    real upper_edge;

    if (bin < 0 || bin >= cmd->sizeHistN
        || !(lower > cmd->rminHist) || !(upper < cmd->rangeN))
        return FALSE;
    if (cmd->useLogHist && gd->deltaRV != NULL) {
        lower_edge = bin == 0 ? cmd->rminHist : gd->deltaRV[bin];
        upper_edge = gd->deltaRV[bin + 1];
    } else if (cmd->useLogHist) {
        const real first_log = cmd->rminHist > 0.0
            ? rlog10(cmd->rminHist)
            : rlog10(cmd->rangeN)
              - (real)cmd->sizeHistN/(real)cmd->logHistBinsPD;
        const real width = cmd->rminHist > 0.0
            ? gd->deltaR : 1.0/(real)cmd->logHistBinsPD;
        lower_edge = rpow(10.0, first_log + (real)bin*width);
        upper_edge = rpow(10.0, first_log + (real)(bin + 1)*width);
    } else {
        lower_edge = cmd->rminHist + (real)bin*gd->deltaR;
        upper_edge = lower_edge + gd->deltaR;
    }
    return lower >= lower_edge && upper < upper_edge;
}
...
static bool shear_load_unit3(const real *position, compute_vector unit)
{
#ifdef SHEAR_SPHERE_BINARY_POSITIONS_UNIT
    unit[0] = position[0];
    unit[1] = position[1];
    unit[2] = position[2];
    return TRUE;
#else
    return shear_unit3(position, unit);
#endif
}

static bool shear_spherical_east(const real *unit, compute_vector east)
{
...
{
#ifdef SHEAR_SPHERE_BODY_POSITIONS_UNIT
    work->pivot_unit[0] = Pos(work->pivot)[0];
    work->pivot_unit[1] = Pos(work->pivot)[1];
    work->pivot_unit[2] = Pos(work->pivot)[2];
    return shear_spherical_basis(work->pivot_unit, work->pivot_east,
                                 work->pivot_north);
#else
    return shear_load_unit3(Pos(work->pivot), work->pivot_unit)
        && shear_spherical_basis(work->pivot_unit, work->pivot_east,
                                 work->pivot_north);
#endif
}

static bool shear_spherical_phase_unit(
        const shear_pivot_workspace *work, const real *position,
        shear_complex *phase)
{
    const real projection = shear_dot3(work->pivot_unit, position);
    const real tangent_norm2 = MAX(0.0, 1.0 - projection*projection);
    const real tangent_norm = rsqrt(tangent_norm2);

    if (!(tangent_norm > 64.0*DBL_EPSILON) || !isfinite(tangent_norm))
        return FALSE;
    phase->re = shear_dot3(position, work->pivot_east)/tangent_norm;
    phase->im = shear_dot3(position, work->pivot_north)/tangent_norm;
...
{
#ifdef SHEAR_SPHERE_BINARY_POSITIONS_UNIT
    return shear_spherical_phase_unit(work, position, phase);
#else
    compute_vector tangent;
    real projection;
    real tangent_norm;

    projection = shear_dot3(work->pivot_unit, position);
...
static bool shear_spherical_phase_node(
        const shear_pivot_workspace *work, nodeptr q,
        shear_complex *phase)
{
#ifdef SHEAR_SPHERE_BODY_POSITIONS_UNIT
    if (Type(q) != CELL)
        return shear_spherical_phase_unit(work, Pos(q), phase);
#endif
    return shear_spherical_phase(work, Pos(q), phase);
}

static bool shear_transport_rotation_between_frames(
        const real *pivot_unit, const real *pivot_east,
        const real *pivot_north, const real *q_unit,
        const real *q_east,
        shear_complex *rotation)
{
    real dot;
    real denominator;
    real q_east_dot_pivot;
    real c;
    real s;
    ...
    if (!(denominator > 64.0*DBL_EPSILON) || !isfinite(denominator))
        return FALSE;
    q_east_dot_pivot = shear_dot3(q_east, pivot_unit);
#ifdef SHEAR_HAVE_VECTOR2
    {
        shear_vector2 projections = {
            shear_dot3(pivot_east, q_unit)/denominator,
            shear_dot3(pivot_north, q_unit)/denominator
        };
        shear_vector2 orientation = {
            shear_dot3(q_east, pivot_east),
            shear_dot3(q_east, pivot_north)
        };
        orientation -= projections*q_east_dot_pivot;
        c = orientation[0];
        s = orientation[1];
    }
#else
    c = shear_dot3(q_east, pivot_east)
        - shear_dot3(pivot_east, q_unit)/denominator*q_east_dot_pivot;
    s = shear_dot3(q_east, pivot_north)
        - shear_dot3(pivot_north, q_unit)/denominator*q_east_dot_pivot;
#endif
...
static bool shear_transport_rotation_between_units(
        const real *pivot_unit, const real *pivot_east,
        const real *pivot_north, const real *q_unit,
        shear_complex *rotation)
{
    ...
    return shear_spherical_east(q_unit, q_east)
        && shear_transport_rotation_between_frames(
               pivot_unit, pivot_east, pivot_north,
               q_unit, q_east, rotation);
}
        ...
        shear_complex *rotation)
{
    ...
    return shear_load_unit3(neighbor_position, neighbor_unit)
        && shear_transport_rotation_between_units(
               pivot_unit, pivot_east, pivot_north, neighbor_unit, rotation);
}

static bool shear_transport_rotation_to_pivot_node(
        const real *pivot_unit, const real *pivot_east,
        const real *pivot_north, nodeptr neighbor,
        shear_complex *rotation)
{
#if defined(SHEAR_SPHERE_BODY_POSITIONS_UNIT) \
    && !defined(SHEAR_SPHERE_BINARY_POSITIONS_UNIT)
    if (Type(neighbor) != CELL)
        return shear_transport_rotation_between_units(
            pivot_unit, pivot_east, pivot_north, Pos(neighbor), rotation);
#endif
    return shear_transport_rotation_to_pivot(
        pivot_unit, pivot_east, pivot_north, Pos(neighbor), rotation);
}
                                     ...
                                     shear_complex *transported)
{
    ...
    if (work->profile == NULL) {
        if (!shear_transport_rotation_to_pivot_node(
    ...
    *transported = shear_mul(input, rotation);
    const bool valid = isfinite(transported->re) && isfinite(transported->im);
    shear_profile_sample_end(sample, started, active);
    return valid;
}

static bool shear_transport_rotation_profiled(
        shear_pivot_workspace *work, const real *position,
        shear_complex *rotation)
{
    shear_profile_sample *sample;
    double started = 0.0;
    ...
    sample = &work->profile->transport;
    active = shear_profile_sample_begin(sample, &started);
    valid = shear_transport_rotation_to_pivot(
        work->pivot_unit, work->pivot_east, work->pivot_north,
        position, rotation);
...
static bool shear_transport_rotation_node_profiled(
        shear_pivot_workspace *work, nodeptr q,
        shear_complex *rotation)
{
    shear_profile_sample *sample;
    double started = 0.0;
    ...
    if (work->profile == NULL)
        return shear_transport_rotation_to_pivot_node(
            work->pivot_unit, work->pivot_east, work->pivot_north,
            q, rotation);
    sample = &work->profile->transport;
    active = shear_profile_sample_begin(sample, &started);
    valid = shear_transport_rotation_to_pivot_node(
        work->pivot_unit, work->pivot_east, work->pivot_north,
        q, rotation);
    shear_profile_sample_end(sample, started, active);
    return valid;
        ...
        struct cmdline_data *cmd, nodeptr q, real qsize)
{
    ...
    for (child_index = 0; child_index < NSUB; child_index++) {
        ...
        if (!(weight >= 0.0) || !isfinite(weight)
            || !shear_transport_rotation_to_pivot_node(
...
static void shear_accumulate_3pcf_sample_core(
        shear_pivot_workspace *work, int bin, shear_complex phase,
        shear_complex weighted_gamma, real weight,
        shear_complex gamma2_sum, real gamma_abs2_sum, real weight2_sum)
{
    shear_complex *g_ring = work->g_ring_active
        + (size_t)bin*(size_t)(2*work->ring_max + 1)
        + (size_t)work->ring_max;
    shear_complex *w_ring = work->w_ring_active
        + (size_t)bin*(size_t)(work->ring_max + 1);
    shear_complex phase_power = shear_make(1.0, 0.0);
    int order;

    shear_accumulate_complex(&g_ring[0], weighted_gamma);
    w_ring[0].re += weight;
    for (order = 1; order <= work->ring_max; order++) {
        phase_power = shear_mul(phase_power, phase);
        shear_accumulate_complex_product(
            &g_ring[order], weighted_gamma, phase_power);
        shear_accumulate_complex_product(
            &g_ring[-order], weighted_gamma, shear_conj(phase_power));
        shear_accumulate_complex_scaled(&w_ring[order], phase_power, weight);
    }
                                         ...
                                         real weight2_sum)
{
    ...
    if (work->profile == NULL) {
        shear_accumulate_3pcf_sample_core(
            work, bin, phase, weighted_gamma, weight, gamma2_sum,
            gamma_abs2_sum, weight2_sum);
        return;
    }
    sample = &work->profile->ring;
    active = shear_profile_sample_begin(sample, &started);
    shear_accumulate_3pcf_sample_core(
        work, bin, phase, weighted_gamma, weight, gamma2_sum,
        gamma_abs2_sum, weight2_sum);
                                       ...
                                       int bin, shear_complex phase)
{
    ...
    if (!shear_transport_rotation_node_profiled(work, q, &rotation))
        return;
        ...
        for (bin2 = 0; bin2 < work->bins; bin2++) {
            for (order = -2*nmax; order <= 2*nmax; order++) {
                shear_complex value = shear_mul(
                    shear_weight_ring_value(
                        work->w_ring_first, order, bin1, work->ring_max),
                    shear_conj(shear_weight_ring_value(
                        work->w_ring_second, order, bin2, work->ring_max)));
...
static void shear_reduce_pivot_profiled(shear_result_accumulator *result,
                                        shear_pivot_workspace *work,
                                        int nmax, bool run_2pcf,
                                        bool run_3pcf)
{
    const double started = work->profile != NULL
        ? shear_profile_wall_time() : 0.0;

    shear_reduce_pivot(result, work, nmax, run_2pcf, run_3pcf);
    if (work->profile != NULL)
        work->profile->reduce_seconds += shear_profile_wall_time() - started;
}

static void shear_clear_pivot_workspace(shear_pivot_workspace *work,
                                        size_t ring_count,
                                        size_t weight_ring_count, int bins,
                                        bool run_3pcf, bool scan_2pcf,
                                        bool distinct_second_leg)
{
    const double started = work->profile != NULL
        ? shear_profile_wall_time() : 0.0;
    ...
    if (run_3pcf) {
        ...
        memset(work->w_ring_first, 0,
               weight_ring_count*sizeof(*work->w_ring_first));
        if (distinct_second_leg) {
            memset(work->g_ring_second, 0,
                   ring_count*sizeof(*work->g_ring_second));
            memset(work->w_ring_second, 0,
                   weight_ring_count*sizeof(*work->w_ring_second));
        }
    ...
    if (work->profile != NULL)
        work->profile->clear_seconds += shear_profile_wall_time() - started;
}

static void shear_print_profiles(const shear_profile_counters *profiles,
                                 int threads)
{
    int rank = 0;
    ...
    for (int thread = 0; thread < threads; thread++) {
        const shear_profile_counters *profile = &profiles[thread];
        const double radial = shear_profile_estimate(&profile->radial);
        const double transport = shear_profile_estimate(&profile->transport);
        const double ring = shear_profile_estimate(&profile->ring);

        max_elapsed = MAX(max_elapsed, profile->elapsed_seconds);
        total_clear += profile->clear_seconds;
        total_walk += profile->walk_seconds;
        total_reduce += profile->reduce_seconds;
        total_radial += radial;
        total_transport += transport;
        total_ring += ring;
        fprintf(stderr,
                "SHEAR_PROFILE engine=%s rank=%d thread=%d pivots=%" PRIu64
                " elapsed=%.6f clear=%.6f walk=%.6f reduce=%.6f"
                " radial_est=%.6f transport_est=%.6f ring_est=%.6f"
                " radial_calls=%" PRIu64 " transport_calls=%" PRIu64
                " ring_calls=%" PRIu64 "\n",
                SHEAR_ENGINE_NAME, rank, thread, profile->pivots,
                profile->elapsed_seconds, profile->clear_seconds,
                profile->walk_seconds, profile->reduce_seconds,
                radial, transport, ring, profile->radial.calls,
                profile->transport.calls, profile->ring.calls);
    }
    fprintf(stderr,
            "SHEAR_PROFILE engine=%s rank=%d aggregate threads=%d"
            " max_elapsed=%.6f"
            " cpu_clear=%.6f cpu_walk=%.6f cpu_reduce=%.6f"
            " cpu_radial_est=%.6f cpu_transport_est=%.6f"
            " cpu_ring_est=%.6f\n",
            SHEAR_ENGINE_NAME, rank, threads, max_elapsed,
            total_clear, total_walk,
            total_reduce, total_radial, total_transport, total_ring);
}
...
#ifdef SHEAR_MPI_ENABLED
static int shear_mpi_reduce_results(struct cmdline_data *cmd,
                                    struct global_data *gd,
                                    int bins, size_t gamma_count,
                                    size_t denominator_count,
                                    bool run_2pcf, bool run_3pcf)
{
    const double started = getenv("CBALLS_SHEAR_PROFILE") != NULL
        ? shear_profile_wall_time() : 0.0;
    const size_t bin_count = (size_t)bins;
    size_t real_count = 0;
    size_t two_gamma = 0;
    size_t two_denominator = 0;
    size_t offset = 0;
    real *packed = NULL;
    INTEGER counters[2] = {gd->nbbcalc, gd->nbccalc};
    int allocation_status = SUCCESS;

    if (SHEAR_MPI_SIZE() <= 1) return SUCCESS;
    if ((run_2pcf
         && shear_size_mul(5, bin_count, &real_count) == FAILURE)
        || (run_3pcf
            && (shear_size_mul(2, gamma_count, &two_gamma) == FAILURE
                || shear_size_mul(2, denominator_count,
                                  &two_denominator) == FAILURE
                || shear_size_add(real_count, two_gamma,
                                  &real_count) == FAILURE
                || shear_size_add(real_count, two_denominator,
                                  &real_count) == FAILURE))) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 SHEAR_ENGINE_NAME ": MPI reduction size overflow");
        return FAILURE;
    }
    if (real_count > 0) {
        if (real_count > SIZE_MAX/sizeof(*packed)
            || (packed = malloc(real_count*sizeof(*packed))) == NULL) {
            snprintf(cmd->error_message, _ERRORMSGSIZE_,
                     SHEAR_ENGINE_NAME
                     ": MPI reduction workspace allocation failed");
            allocation_status = FAILURE;
        }
        if (SHEAR_MPI_CONSENSUS(
                cmd, allocation_status,
                "MPI shear reduction workspace allocation") == FAILURE) {
            free(packed);
            return FAILURE;
        }
        if (run_2pcf) {
            memcpy(packed + offset, gd->histShearXiPlusRe,
                   bin_count*sizeof(*packed));
            offset += bin_count;
            memcpy(packed + offset, gd->histShearXiPlusIm,
                   bin_count*sizeof(*packed));
            offset += bin_count;
            memcpy(packed + offset, gd->histShearXiMinusRe,
                   bin_count*sizeof(*packed));
            offset += bin_count;
            memcpy(packed + offset, gd->histShearXiMinusIm,
                   bin_count*sizeof(*packed));
            offset += bin_count;
            memcpy(packed + offset, gd->histShearXiWeight,
                   bin_count*sizeof(*packed));
            offset += bin_count;
        }
        if (run_3pcf) {
            memcpy(packed + offset, gd->histShearGammaNumeratorRe,
                   gamma_count*sizeof(*packed));
            offset += gamma_count;
            memcpy(packed + offset, gd->histShearGammaNumeratorIm,
                   gamma_count*sizeof(*packed));
            offset += gamma_count;
            memcpy(packed + offset, gd->histShearDenominatorRe,
                   denominator_count*sizeof(*packed));
            offset += denominator_count;
            memcpy(packed + offset, gd->histShearDenominatorIm,
                   denominator_count*sizeof(*packed));
            offset += denominator_count;
        }
        if (SHEAR_MPI_REDUCE_REALS(cmd, packed, real_count) == FAILURE) {
            free(packed);
            return FAILURE;
        }
        if (SHEAR_MPI_IS_ROOT()) {
            offset = 0;
            if (run_2pcf) {
                memcpy(gd->histShearXiPlusRe, packed + offset,
                       bin_count*sizeof(*packed));
                offset += bin_count;
                memcpy(gd->histShearXiPlusIm, packed + offset,
                       bin_count*sizeof(*packed));
                offset += bin_count;
                memcpy(gd->histShearXiMinusRe, packed + offset,
                       bin_count*sizeof(*packed));
                offset += bin_count;
                memcpy(gd->histShearXiMinusIm, packed + offset,
                       bin_count*sizeof(*packed));
                offset += bin_count;
                memcpy(gd->histShearXiWeight, packed + offset,
                       bin_count*sizeof(*packed));
                offset += bin_count;
            }
            if (run_3pcf) {
                memcpy(gd->histShearGammaNumeratorRe, packed + offset,
                       gamma_count*sizeof(*packed));
                offset += gamma_count;
                memcpy(gd->histShearGammaNumeratorIm, packed + offset,
                       gamma_count*sizeof(*packed));
                offset += gamma_count;
                memcpy(gd->histShearDenominatorRe, packed + offset,
                       denominator_count*sizeof(*packed));
                offset += denominator_count;
                memcpy(gd->histShearDenominatorIm, packed + offset,
                       denominator_count*sizeof(*packed));
            }
        }
        free(packed);
    }
    if (SHEAR_MPI_REDUCE_INTEGERS(cmd, counters, 2) == FAILURE)
        return FAILURE;
    if (SHEAR_MPI_IS_ROOT()) {
    ...
    if (started > 0.0)
        fprintf(stderr,
                "SHEAR_PROFILE engine=%s rank=%d mpi_reduce=%.6f"
                " real_values=%zu integer_values=2\n",
                SHEAR_ENGINE_NAME, SHEAR_MPI_RANK(),
                shear_profile_wall_time() - started, real_count);
    return SUCCESS;
}
#endif
...
#include "../shear_sphere_binary_2balls/shear_sphere_binary_scan.h"
    ...
    || defined(SHEAR_SPHERE_NATIVE_FRONTIER_SCHEDULER)
#define SHEAR_SPHERE_FRONTIER_SCHEDULER 1
#endif
                                       ...
                                       int cat1, int cat2, int cat3)
{
    ...
    size_t ring_count = 0;
    size_t weight_ring_count = 0;
    size_t threaded_ring_count = 0;
    size_t threaded_weight_ring_count = 0;
    size_t threaded_bin_count = 0;
#ifdef BALLS4SCANLEV
    size_t accumulator_stride = 0;
    size_t threaded_accumulator_count = 0;
#endif
    ...
    kd_shear_frontier_schedule frontier_schedule = {0};
#else
    shear_native_frontier_schedule frontier_schedule = {0};
#endif
    int frontier_rank = 0;
    int frontier_ranks = 1;
    INTEGER frontier_local_task_count = 0;
    INTEGER *frontier_local_tasks = NULL;
#else
...
#ifdef SHEAR_MPI_ENABLED
        dual_status = SHEAR_MPI_CONSENSUS(
            cmd, dual_status, "MPI symmetric dual-tree 2PCF allocation");
#endif
        if (dual_status == FAILURE) {
            kd_shear_release_trees(kd_pivot_tree, kd_first_tree,
                                   kd_second_tree);
            return FAILURE;
        }
        gd->nbbcalc = 0;
        gd->nbccalc = 0;
        dual_status = kd_shear_dual_tree_2pcf(
            cmd, gd, kd_pivot_tree, kd_first_tree, cat1 == cat2, bins);
#ifdef SHEAR_MPI_ENABLED
        dual_status = SHEAR_MPI_CONSENSUS(
            cmd, dual_status, "MPI symmetric dual-tree 2PCF traversal");
#endif
        if (dual_status == FAILURE) {
            shear_clear_results(gd);
            kd_shear_release_trees(kd_pivot_tree, kd_first_tree,
...
#ifdef SHEAR_MPI_ENABLED
        if (shear_mpi_reduce_results(
                cmd, gd, bins, 0, 0, TRUE, FALSE) == FAILURE) {
            shear_clear_results(gd);
            kd_shear_release_trees(kd_pivot_tree, kd_first_tree,
                                   kd_second_tree);
            return FAILURE;
    ...
    if (run_2pcf && !run_3pcf
        ...
        if (dual_eligible) {
            ...
            dual_status = SHEAR_MPI_CONSENSUS(
                cmd, dual_status,
                "MPI native symmetric dual-tree 2PCF allocation");
#endif
            if (dual_status == FAILURE)
                return FAILURE;
            gd->nbbcalc = 0;
            gd->nbccalc = 0;
            dual_status = shear_sphere_dual_tree_2pcf(
                    cmd, gd, (nodeptr)roottable[cat1],
                    (nodeptr)roottable[cat2], cat1 == cat2, bins);
#ifdef SHEAR_MPI_ENABLED
            dual_status = SHEAR_MPI_CONSENSUS(
                cmd, dual_status,
                "MPI native symmetric dual-tree 2PCF traversal");
#endif
            if (dual_status == FAILURE) {
                shear_clear_results(gd);
                return FAILURE;
...
#endif
#ifdef BALLS4SCANLEV
#ifndef SHEAR_SPHERE_FRONTIER_SCHEDULER
#ifdef SHEAR_SPHERE_BINARY_TWO_BALLS
    ...
    if (threads < 1)
        threads = 1;
#if defined(SHEAR_SPHERE_FRONTIER_SCHEDULER) \
    && defined(SHEAR_MPI_ENABLED)
    frontier_rank = SHEAR_MPI_RANK();
    frontier_ranks = MAX(1, SHEAR_MPI_SIZE());
#endif
        ...
        || (run_3pcf
            && (shear_size_mul((size_t)bins, (size_t)bins,
                ...
                || shear_size_mul((size_t)(2*ring_max + 1), (size_t)bins,
                                  &ring_count) == FAILURE
                || shear_size_mul((size_t)(ring_max + 1), (size_t)bins,
                                  &weight_ring_count) == FAILURE
                || shear_size_mul((size_t)threads, ring_count,
                                  &threaded_ring_count) == FAILURE
                || shear_size_mul((size_t)threads, weight_ring_count,
                                  &threaded_weight_ring_count) == FAILURE))
#ifdef BALLS4SCANLEV
        || (run_2pcf
    ...
    if (run_2pcf && run_3pcf
        && (!cballs_opt_smooth_pivot(cmd) || gd->rsmooth[0] == 0.0)
        && ipmin == 1 && ipmax[cat1] == nbody[cat1]) {
        int dual_status = kd_shear_dual_tree_2pcf(
...
#define SHEAR_ALLOC_WORK(pointer, count, label)                          \
    ...
    if (run_3pcf) {
        ...
        SHEAR_ALLOC_WORK(w_ring_first_all, threaded_weight_ring_count,
                         "thread first W rings");
        if (cat3 != cat2) {
            SHEAR_ALLOC_WORK(g_ring_second_all, threaded_ring_count,
                             "thread second G rings");
            SHEAR_ALLOC_WORK(w_ring_second_all, threaded_weight_ring_count,
    ...
    if (profile_enabled)
        SHEAR_ALLOC_WORK(profile_all, (size_t)threads,
                         "thread shear profiles");
#ifdef BALLS4SCANLEV
#ifdef SHEAR_SPHERE_FRONTIER_SCHEDULER
    {
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
        const INTEGER target = kd_shear_pivot_task_target(
            threads, frontier_ranks, accumulator_stride);
        fcfc_balltreeptr second_frontier_tree =
            run_3pcf && cat3 != cat2 ? kd_second_tree : NULL;
#else
        const INTEGER target = shear_native_pivot_task_target(
            threads, frontier_ranks, accumulator_stride);
        nodeptr second_frontier_root = run_3pcf && cat3 != cat2
            ? (nodeptr)roottable[cat3] : NULL;
#endif
        const double frontier_started = profile_enabled
            ? shear_profile_wall_time() : 0.0;

#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
        if (kd_shear_build_frontier_schedule(
                cmd, kd_pivot_tree, kd_first_tree, second_frontier_tree,
                target, frontier_ranks, &frontier_schedule) == FAILURE) {
            goto fail;
        }
#else
        if (shear_native_build_frontier_schedule(
                cmd, gd, (nodeptr)roottable[cat1],
                btable[cat1] + ipmin - 1,
                btable[cat1] + ipmax[cat1],
                (nodeptr)roottable[cat2], second_frontier_root,
                target, frontier_ranks, &frontier_schedule) == FAILURE) {
            goto fail;
        }
#endif
        for (INTEGER task = 0; task < frontier_schedule.task_count; task++)
            if (frontier_schedule.tasks[task].owner == frontier_rank)
                frontier_local_task_count++;
        if (profile_enabled)
            fprintf(stderr,
                    "SHEAR_PROFILE engine=%s rank=%d frontier_target=%"
                    INTEGER_FMT " frontier_tasks=%" INTEGER_FMT
                    " local_tasks=%" INTEGER_FMT "\n",
                    SHEAR_ENGINE_NAME, frontier_rank, target,
                    frontier_schedule.task_count, frontier_local_task_count);
        if (profile_enabled)
            fprintf(stderr,
                    "SHEAR_PROFILE engine=%s rank=%d frontier_schedule=%.6f"
                    " pivots=%zu neighbor_roots=%zu/%zu\n",
                    SHEAR_ENGINE_NAME, frontier_rank,
                    shear_profile_wall_time() - frontier_started,
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
                    (size_t)kd_pivot_tree->npoint,
#else
                    frontier_schedule.pivot_count,
#endif
                    frontier_schedule.first_node_count,
                    frontier_schedule.second_node_count);
        SHEAR_ALLOC_WORK(frontier_local_tasks,
                         (size_t)MAX((INTEGER)1,
                                     frontier_local_task_count),
                         "rank-local pivot task indices");
        for (INTEGER task = 0; task < frontier_schedule.task_count; task++)
            if (frontier_schedule.tasks[task].owner == frontier_rank)
                frontier_local_tasks[
                    frontier_schedule.tasks[task].local_index] = task;
        if (shear_size_mul(
                (size_t)MAX((INTEGER)1, frontier_local_task_count),
                accumulator_stride,
                &threaded_accumulator_count) == FAILURE) {
            if (cmd->error_message[0] == '\0')
                snprintf(cmd->error_message, _ERRORMSGSIZE_,
                         SHEAR_ENGINE_NAME
                         ": task histogram size overflow");
            goto fail;
        }
        verb_print(
            cmd->verbose,
            SHEAR_ENGINE_NAME ": adaptive pivot frontier has %"
            INTEGER_FMT " tasks, %.1f/%.1f neighbor roots per task\n",
            frontier_schedule.task_count,
            frontier_schedule.task_count > 0
                ? (double)frontier_schedule.first_node_count
                  / (double)frontier_schedule.task_count : 0.0,
            frontier_schedule.task_count > 0
                ? (double)frontier_schedule.second_node_count
                  / (double)frontier_schedule.task_count : 0.0);
    }
#endif
    SHEAR_ALLOC_WORK(block_accumulator_all, threaded_accumulator_count,
                     "task result accumulators");
...
#ifdef BALLS4SCANLEV
#ifdef SHEAR_SPHERE_FRONTIER_SCHEDULER
#define SHEAR_SCAN_SHARED shared(frontier_schedule,block_accumulator_all,    \
                                 global_accumulator,accumulator_stride,      \
                                 gamma_count,denominator_count,              \
                                 frontier_local_tasks,                       \
                                 frontier_local_task_count)
#else
...
#pragma omp parallel private(ip) SHEAR_SCAN_SHARED
    {
        ...
        work.w_ring_first = run_3pcf
            ? w_ring_first_all + (size_t)thread_id*weight_ring_count : NULL;
        work.g_ring_second = run_3pcf
            ? (cat3 == cat2 ? work.g_ring_first
                            : g_ring_second_all
                              + (size_t)thread_id*ring_count)
            : NULL;
        work.w_ring_second = run_3pcf
            ? (cat3 == cat2 ? work.w_ring_first
                            : w_ring_second_all
                              + (size_t)thread_id*weight_ring_count)
            : NULL;
...
#pragma omp for schedule(dynamic,1)
        for (INTEGER local_task = 0;
             local_task < frontier_local_task_count; local_task++) {
            const INTEGER task_index = frontier_local_tasks[local_task];
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
            const kd_shear_pivot_task *task =
                &frontier_schedule.tasks[task_index];
#else
            ...
            shear_bind_local_accumulator(
                block_accumulator_all
                    + (size_t)task->local_index*accumulator_stride,
            ...
            for (INTEGER point = task->first; point <= task->last; point++) {
                ...
                if (work.pivot < btable[cat1] + ipmin - 1
                    || work.pivot >= btable[cat1] + ipmax[cat1])
                    continue;
#else
            for (INTEGER point = 0; point < task->pivot_count; point++) {
                work.pivot = frontier_schedule.pivots[
                    task->pivot_offset + (size_t)point];
#endif
#else
        shear_bind_local_accumulator(
...
#pragma omp for schedule(dynamic,1) ordered
        for (INTEGER block = 0; block < block_count; block++) {
            ...
            const INTEGER count = MIN((INTEGER)SHEAR_OMP_PIVOT_BLOCK_SIZE,
                                      pivot_count - first);
            const bool block_owned =
#ifdef SHEAR_MPI_ENABLED
                SHEAR_MPI_TASK_OWNED(block);
#else
                TRUE;
#endif
            shear_zero_accumulator(&accumulator, (size_t)bins, gamma_count,
                                   denominator_count);
            for (INTEGER offset = 0; block_owned && offset < count; offset++) {
...
#pragma omp for schedule(static,1) ordered
        for (ip = ipmin - 1; ip < ipmax[cat1]; ip++) {
            const bool pivot_owned =
#ifdef SHEAR_MPI_ENABLED
                SHEAR_MPI_TASK_OWNED(
                    ip - (ipmin - 1));
#else
                TRUE;
#endif
...
#endif
            shear_clear_pivot_workspace(
                &work, ring_count, weight_ring_count, bins,
                run_3pcf, scan_2pcf, cat3 != cat2);
            const double profile_walk_started = work.profile != NULL
                ? shear_profile_wall_time() : 0.0;
...
#ifdef SHEAR_SPHERE_BINARY_TWO_BALLS
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
                for (INTEGER root = 0; root < task->first_count; root++)
                    kd_shear_walk(
                        &work, kd_first_tree,
                        frontier_schedule.first_nodes[
                            task->first_offset + (size_t)root],
                        scan_2pcf, run_3pcf);
#else
...
#ifdef SHEAR_SPHERE_NATIVE_FRONTIER_SCHEDULER
                for (INTEGER root = 0; root < task->first_count; root++) {
                    nodeptr neighbor = frontier_schedule.first_nodes[
                        task->first_offset + (size_t)root];
                    const real neighbor_size = Type(neighbor) == CELL
                        ? (real)Size(neighbor) : 0.0;

                    if (scan_2pcf && run_3pcf)
                        shear_walk_tree_both(
                            &work, neighbor, neighbor_size);
                    else if (scan_2pcf)
                        shear_walk_tree_2pcf(
                            &work, neighbor, neighbor_size);
                    else
                        shear_walk_tree_3pcf(
                            &work, neighbor, neighbor_size);
                }
#else
                if (scan_2pcf && run_3pcf)
...
#ifdef SHEAR_SPHERE_BINARY_TWO_BALLS
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
                    for (INTEGER root = 0; root < task->second_count; root++)
                        kd_shear_walk(
                            &work, kd_second_tree,
                            frontier_schedule.second_nodes[
                                task->second_offset + (size_t)root],
                            FALSE, TRUE);
#else
...
#ifdef SHEAR_SPHERE_NATIVE_FRONTIER_SCHEDULER
                    for (INTEGER root = 0; root < task->second_count; root++) {
                        nodeptr neighbor = frontier_schedule.second_nodes[
                            task->second_offset + (size_t)root];
                        const real neighbor_size = Type(neighbor) == CELL
                            ? (real)Size(neighbor) : 0.0;
            ...
            if (work.profile != NULL) {
                work.profile->walk_seconds +=
                    shear_profile_wall_time() - profile_walk_started;
                work.profile->pivots++;
            }

#ifdef BALLS4SCANLEV
                shear_reduce_pivot_profiled(&accumulator, &work, nmax,
                                            scan_2pcf, run_3pcf);
            }
#ifndef SHEAR_SPHERE_FRONTIER_SCHEDULER
#pragma omp ordered
...
#if defined(BALLS4SCANLEV) \
    && defined(SHEAR_SPHERE_FRONTIER_SCHEDULER)
    for (INTEGER task_index = 0;
         task_index < frontier_schedule.task_count; task_index++) {
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
        const kd_shear_pivot_task *task =
            &frontier_schedule.tasks[task_index];
#else
        const shear_native_pivot_task *task =
            &frontier_schedule.tasks[task_index];
#endif
        shear_result_accumulator task_accumulator;

        if (task->owner != frontier_rank) continue;
        shear_bind_local_accumulator(
            block_accumulator_all
                + (size_t)task->local_index*accumulator_stride,
            (size_t)bins, gamma_count, denominator_count,
            scan_2pcf, run_3pcf, &task_accumulator);
        shear_merge_accumulator(
            &global_accumulator, &task_accumulator, (size_t)bins,
            gamma_count, denominator_count);
    }
#endif

#ifdef SHEAR_MPI_ENABLED
    if (shear_mpi_reduce_results(cmd, gd, bins, gamma_count,
                                 denominator_count, run_2pcf, run_3pcf)
        == FAILURE)
        goto fail;
    if (!SHEAR_MPI_IS_ROOT()) {
        gd->cpusearch = CPUTIME - cpu_start;
...
#ifdef SHEAR_SPHERE_FRONTIER_SCHEDULER
        free(frontier_local_tasks);
#ifdef SHEAR_SPHERE_BINARY_FRONTIER_SCHEDULER
        kd_shear_release_frontier_schedule(&frontier_schedule);
#else
        shear_native_release_frontier_schedule(&frontier_schedule);
#endif
#else
        free(pivot_order);
#endif
...
#undef SHEAR_SPHERE_FRONTIER_SCHEDULER
#endif /* NDIM >= 2 */
