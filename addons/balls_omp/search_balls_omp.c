...
local int sumnode_balls6_omp(struct cmdline_data* cmd, struct  global_data* gd,
                              cellptr, cellptr, bodyptr, INTEGER *,
            INTEGER *,  INTEGER *,
            gdhistptr_omp_balls6, gdhistptr_sincos_omp_balls6);
local int sumcell_balls6_omp(struct cmdline_data* cmd, struct  global_data* gd,
                              cellptr, cellptr, bodyptr, INTEGER *,
            INTEGER *,  INTEGER *,
            gdhistptr_omp_balls6, gdhistptr_sincos_omp_balls6);
local int sumcellcell_balls6_omp(struct cmdline_data* cmd,
                                  struct  global_data* gd,
...
// search=balls-omp
global int searchcalc_balls_omp(struct cmdline_data* cmd,
                                 struct  global_data* gd,
                                 bodyptr *btable,
...
//B BALLS4
    ...
    if (cmd->useLogHist==FALSE)
        cBALLS_FAIL(cmd,
                    "CheckParameters: can´t have normal hist and BALLS definition (useLogHist=%d)\nSet useLogHist = true\n",
                              cmd->useLogHist);
...
#ifdef SINCOS
#ifndef TPCF
    verb_print(cmd->verbose, "warning: sincos base... without defining TPCF?...\n");
#endif
#endif
...
#pragma omp parallel default(none)   \
    shared(cmd,gd,btable,nbody,roottable,ipmin,ipmax,nodetablescanlev, \
           nodetablescanlev_root,rootnode,imiss, cat1, cat2 )
#endif // ! SMOOTHPIVOT
...
//#ifdef TPCF
      if (gd->computeTPCF) {                       // cmd->computeTPCF not defined anymore
                                                    //  use #ifdef THREEPCFCONVERGENCE ...
#if NDIM == 3
#ifdef TREENODEALLBODIES
...
#ifdef TREENODEALLBODIES
//B check cmd->scanLevelRoot==0 this condition
//      because nodes to do this task are not allocating any in treeload
            nodeptr q;
            int j;
...
//                verb_print_debug(1, "\nAqui voy (1): %g %g %g\n",Pos(p)[0],Pos(p)[1],Pos(p)[2]);
            ...
            if (walktree_balls6_omp(cmd, gd, histb.active, histb.active + 1,
                    histb.interact, histb.interact + histb.actlen,
                    p, Size(p), Pos(p),
                    &nbbcalcthread, &nbccalcthread, &ncccalcthread,
                    &histb, &histccb, &histbsincos,
                    &ibfcountthread, &nsmoothcountthread, nbody[cat1]) == FAILURE)
                return FAILURE;
//            }
...
#else
        if (cmd->verbose > 2)
            verb_print(cmd->verbose,"%d %e\n",
                       nn, gd->histNNSubXi2pcf[nn]);
#endif
    ...
    if (!reject_balls(cmd, gd, p, q, &dr1, dr)) {
        ...
        if (Type(q) == CELL) {
            if (!cballs_opt_no_one_ball(cmd)) {
                ...
                sumnodes_bb_omp(cmd, gd, p, q, &dr1, dr, hist, histcc,
                                histccsincos,
                                nbbcalcthread, nbccalcthread, ncccalcthread);
        } // ! Type(q) == CELL
    } // ! reject_cell
                     ...
                     if ( ((Nb(*ap)<=gd->nsmooth[0]) || (Size(*ap)<=gd->rminCell[0]))
                         && (dr1 > gd->rminCell[1]) ) {
                             if (np - hist->active >= actsafe)
                                 cBALLS_FAIL(cmd,
                                        "walktree (1): active list overflow\n");
//                                 error("walktree (1): active list overflow\n");
                         ...
                         if (nodes_condition_balls(cmd, gd, p, *ap, &dr1, dr)) {
                             if (!cballs_opt_no_two_balls(cmd)
                                 && Type(p) == CELL ) {
                                 if (sumcellcell_balls6_omp(cmd, gd, (cellptr)(*ap),
                                    (cellptr)*ap+1, p,
                                    nbbcalcthread, nbccalcthread, ncccalcthread,
                                    histcc, histsincos) == FAILURE)
                                     return FAILURE;
                             } else {
                                 if (np - hist->active >= actsafe)
                                     cBALLS_FAIL(cmd,
                                        "walktree (2): active list overflow\n");
...
//E End loop for ap
        ...
        if (np != nptr)
            if (walksub6_omp(cmd, gd, nptr, np, cptr, bptr, p, psize, pmid,
                        nbbcalcthread, nbccalcthread, ncccalcthread,
                            hist, histcc, histsincos, ibfcountthread, nsmoothcountthread, nbody) == FAILURE)
                return FAILURE;
        else {
            if (Type(p) != BODY)
                cBALLS_FAIL(cmd,
                            "walktree: recursion terminated with cell\n");
        ...
        INTEGER *ibfcountthread, INTEGER *nsmoothcountthread, INTEGER nbody)
{
    ...
    if (Type(p) == CELL) {
        for (q = More(p); q != Next(p); q = Next(q)) {
            ...
            if (walktree_balls6_omp(cmd, gd, nptr, np, cptr, bptr, q, psize / 2, nmid,
                    nbbcalcthread, nbccalcthread, ncccalcthread, hist, histcc,
                    histsincos, ibfcountthread, nsmoothcountthread, nbody) == FAILURE)
            return FAILURE;
        }
    } else {
        for (k = 0; k < NDIM; k++)
            nmid[k] = pmid[k] + (Pos(p)[k] < pmid[k] ? - poff : poff);
        if (walktree_balls6_omp(cmd, gd, nptr, np, cptr, bptr, p, psize / 2, nmid,
                    nbbcalcthread, nbccalcthread, ncccalcthread, hist, histcc,
                    histsincos, ibfcountthread, nsmoothcountthread, nbody) == FAILURE)
            return FAILURE;
    }
...
//E
    if (!cballs_opt_no_one_ball(cmd)) {
        if (sumcell_balls6_omp(cmd, gd, hist->interact, cptr, (bodyptr) p0,
                               nbbcalcthread, nbccalcthread, ncccalcthread, &hist1, &hist1sincos) == FAILURE)
            return FAILURE;
    }
    
    if (sumnode_balls6_omp(cmd, gd, bptr, hist->interact + hist->actlen,
                       (bodyptr) p0,
                nbbcalcthread, nbccalcthread, ncccalcthread, &hist1, &hist1sincos) == FAILURE)
        return FAILURE;
...
local int sumcellcell_balls6_omp(struct cmdline_data* cmd, struct  global_data* gd,
