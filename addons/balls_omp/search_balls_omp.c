...
//      verb_print_debug(1, "\nAqui voy (11)\n");
  ...
  // Here we clear threads of: histZetaM, histXi, histNSub,
  //  histN, histNSubXi2pcf, histXi2pcf, histXi2pcf()sub...
...
#ifdef TREENODEBALLS4
                histbsincos.histNSubXi2pcfthreadp[n] = 0.0; // Affects only 2pcf
                histb.histNSubXi2pcfthreadp[n] = 0.0;       // Affects only 2pcf
                histccb.histNSubXi2pcfthreadp[n] = 0.0;     // Affects only 2pcf
#else
                hist.histNSubXi2pcfthreadp[n] = 0.0;        // Affects only 2pcf
                histsincos.histNSubXi2pcfthreadp[n] = 0.0;  // Affects only 2pcf
                histcc.histNSubXi2pcfthreadp[n] = 0.0;      // Affects only 2pcf
#ifdef TREENODEALLBODIES
...
//B kappa Avg Rmin
                for (n = 1; n <= cmd->sizeHistN; n++) {
                    hist.histNSubXi2pcfthreadp[n] =
                                    ((real)NbRmin(p))*hist.histNSubXi2pcfthreadp[n];
                    hist.histNSubXi2pcfthreadtotal[n] +=
                                    hist.histNSubXi2pcfthreadp[n];
                    histsincos.histNSubXi2pcfthreadp[n] =
                            ((real)NbRmin(p))*histsincos.histNSubXi2pcfthreadp[n];
                    histsincos.histNSubXi2pcfthreadtotal[n] +=
                                    histsincos.histNSubXi2pcfthreadp[n];
                    if (scanopt(cmd->options, "smooth-pivot")) {
                        hist.histNSubthread[n] =
                        ((real)NbRmin(p))*hist.histNSubthread[n];
                        histcc.histNSubthread[n] =
                        ((real)NbRmin(p))*histcc.histNSubthread[n];
                        histsincos.histNSubthread[n] =
                        ((real)NbRmin(p))*histsincos.histNSubthread[n];
                    }
                }
//E
            } else {
...
//B kappa Avg Rmin
          gd->histNSubXi2pcftotal[n] += hist.histNSubXi2pcfthreadtotal[n];
//E
...
//#endif // ! TPCF
    ...
    for (nn = 1; nn <= cmd->sizeHistN; nn++) {
        printf("%d %e %e\n", nn,
               gd->histNSubXi2pcf[nn], gd->histNSubXi2pcftotal[nn]);
        gd->histXi2pcf[nn] /= 2.0;
        gd->histNSubXi2pcf[nn] /= 2.0;
//        gd->histXi2pcf[nn] /= MAX(gd->histNSubXi2pcf[nn],1.0);
//B kappa Avg Rmin
        gd->histNSubXi2pcftotal[nn] /= 2.0;
        if (scanopt(cmd->options, "smooth-pivot")) {
            gd->histXi2pcf[nn] /= MAX(gd->histNSubXi2pcftotal[nn],1.0);
        } else {
            gd->histXi2pcf[nn] /= MAX(gd->histNSubXi2pcf[nn],1.0);
        }
//E
...
//B kappa Avg Rmin
        hist->histNSubXi2pcfthreadp[n] = hist->histNSubXi2pcfthreadp[n] + 1.;
//E
        xj = Kappa(p);
        xi = Kappa(q);
#ifdef TREENODEALLBODIES
    ...
    if (cmd->computeTPCF) {
        for (m=1; m<=cmd->mchebyshev+1; m++)
            for (n=1; n<=cmd->sizeHistN; n++) {
                ...
                MAX(histsincos->histNSubthread[n],1.0);
...
//#ifndef LOGHIST
    if (!cmd->useLogHist) {
        //#ifdef KDLIB
        //    gd->histN[1]-=nbody; //Substract diagonal
