...
//B kappa Avg Rmin
    ...
    if(!(outpivots=fopen(pivotsfilePath, "w")))
        error("\nsearchcalc_tc_ggg_omp: error opening file '%s' \n",
              pivotsfilePath);
#endif
//E
    ...
    if (scanopt(cmd->options, "patch-with-all")) {
        verb_print(cmd->verbose,
            "\nsearchcalc_tc_ggg_omp: total number of pixels to be pivots: %ld\n",
            gd->pivotCount);
    }
#endif
    //E
    ...
    verb_print(cmd->verbose,
        "\nsearchcalc_tc_ggg_omp: Total allocated %g MByte storage so far.\n",
...
//E
          ...
          normal_walktree_sincos_N(cmd, gd, btable, cat2,
                                   ...
                                   &nbList, &intList);
          computeBodyProperties_sincos_ggg(cmd, gd, p, nbody[cat1], &hist);
          computeBodyProperties_sincos_ggg_N(cmd, gd, p, nbody[cat1], &histN);
...
//B correction 2025-05-03 :: look for edge-effects
#if defined(NMultipoles) && defined(NONORMHIST)
        if (scanopt(cmd->options, "edge-effects")) {
            printf("NNCounts/thread \t= %ld\n", hist.NNCountsthread);
            printf("NNCountsN/thread \t= %ld\n", histN.NNCountsNthread);
            gd->NNCounts += hist.NNCountsthread;
            gd->NNCountsN += histN.NNCountsNthread;
        }
#endif
//E
*/
...
// ===============================================
    verb_print(cmd->verbose,
        "\n\tsearch_octree_ggg_omp: printing octree-ggg-omp method...\n\n");
...
//                hist->NNCountsthread += hist->histNNSubthread[n];
            }
        }
#endif
//E
...
/*
        ...
        if (scanopt(cmd->options, "edge-effects")) {
            for (n=1; n<=cmd->sizeHistN; n++) {
                hist->histXithreadcos[m][n] /= MAX(hist->histNNSubthread[n],1.0);
                hist->histXithreadsin[m][n] /= MAX(hist->histNNSubthread[n],1.0);
//                hist->NNCountsNthread += hist->histNNSubthread[n];
            }
        }
#endif
//E
...
/*
mat3    (gdl->histZetaMcos[lmx][n1][n2]
     + gdl->histZetaMsin[lmx][n1][n2])
mat4    /(gdlN->histZetaMcos[1][n1][n2]
      + gdlN->histZetaMsin[1][n1][n2])
    
mat4    C1 = (gdlN->histZetaMcos[m-l+1][n1][n2]
          + gdlN->histZetaMsin[m-l+1][n1][n2]
mat4          )/(gdlN->histZetaMcos[1][n1][n2]
             +gdlN->histZetaMsin[1][n1][n2]);
 
mat5 gdl->histZetaM[lmx][n1][n2] = gsl_vector_get(x, l);
