...
#else
    real posp[3];
    coordinate_transformation(cmd, gd, Pos(p)[1], Pos(p)[0], posp);
    dRotation3D(posp, ROTANGLE, ROTANGLE, ROTANGLE, hist->q0);
    DOTPSUBV(hist->drpq2, hist->dr0, posp, hist->q0);
#endif
    //E
...
#define POLARAXIS_MAIN                              \
{                                                   \
    real s, sy; real pr0[3];                        \
    DOTVP(s, dr, hist->dr0);                        \
    cosphi = s/(dr1*hist->drpq);                    \
    CROSSVP(pr0,hist->dr0,hist->posp);              \
    DOTVP(sy, dr, pr0);                             \
    sinphi = rsqrt(1.0 - rsqr(cosphi));             \
    if (sy < 0) sinphi *= -1.0;                     \
    if (rabs(cosphi)>1.0)                           \
