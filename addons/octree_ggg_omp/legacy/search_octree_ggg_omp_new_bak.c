    ...
    )
{
    ...
    if (scanopt(cmd->options, "read-mask")) counters[3] = *ipmask;
#ifndef SMOOTHPIVOT
    (void)unused1;
    (void)unused2;
    (void)unused3;
...
#define UNPACK_ZETA(array, orders)                                         \
        ...
        if (scanopt(cmd->options, "read-mask")) *ipmask = counters[3];
#ifdef SMOOTHPIVOT
        *ipfalse = counters[4];
...
#ifdef NMultipoles
    int gdln_local_status = search_init_gd_sincos_omp_ggg_N(cmd, gd, &gdlN);
...
#pragma omp barrier
      if (thread_id == thread_turn && worker_ready && !allocation_failed) {
#ifdef TWOPCF
        for (n = 1; n <= cmd->sizeHistN; n++) {
...
//E
    ...
    if (scanopt(cmd->options, "no-normalize-HistZeta")
        && scanopt(cmd->options, "edge-corrections")) {
...
#ifdef TWOPCF
    if (scanopt(cmd->options, "compute-HistN")) PRINT_OR_FAIL(PrintHistNN(cmd, gd, &gdl));
