...
#else
#pragma omp parallel default(none)                                          \
    shared(cmd,gd,btable,nbody,roottable,actlen,nodetablescanlevB4, \
           ipmin,ipmax,cat1,cat2,ipfalse,icountNbRmin,icountNbRminOverlap,  \
           gdl, gdlN)
#endif
#endif // ! DEBUG
    ...
    if ( ((nodeptr) p) != q ) {
        if (Type(q) == CELL) {
            ...
            find_nblist_omp(cmd, gd,
                            btable, cat2,
                            p, ((cellptr) q),( (cellptr) q+1), nblist, activenb);
    }
}
...
#else
        if (accept_body(cmd, gd, (bodyptr)p, (nodeptr)q, &dr1, dr)) {
#endif
            gd->nbbcalc += 1;
...
#else
        accept_body(cmd, gd, (bodyptr)p, (nodeptr)q, &dr1, dr);
#endif
        //B kappa Avg Rmin
        if (cballs_opt_smooth_pivot(cmd)) {
            if (dr1<=gd->rsmooth[0]) {
...
#else
                    accept_body(cmd, gd, (bodyptr)p, (nodeptr)h, &dr1_h, dr_h);
#endif
