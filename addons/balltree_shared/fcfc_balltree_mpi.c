...
#define CBALLS_MPI_STATE (cballs_runtime_current()->mpi[CBALLS_MPI_FCFC_BALLTREE_MPI])
...
{
    return cballs_mpi_shared_prepare(&CBALLS_MPI_STATE, cmd, gd, !(!fcfc_balltree_mpi_selected(cmd)));
}
...
{
    return cballs_mpi_shared_consensus(&CBALLS_MPI_STATE, cmd, !(!fcfc_balltree_mpi_selected(cmd)), local_status, operation);
}
