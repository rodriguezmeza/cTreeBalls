                                   ...
                                   struct global_data *gd)
{
    ...
    if (finalized) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 "octree-2balls-mpi cannot start after MPI_Finalize");
        return FAILURE;
    }
    ...
    if (!initialized) {
        ...
        if (atexit(finalize_at_exit) != 0) {
            snprintf(cmd->error_message, _ERRORMSGSIZE_,
                     "octree-2balls-mpi could not register MPI cleanup");
            finalize_at_exit();
            return FAILURE;
        }
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 "octree-2balls-mpi requires MPI_THREAD_FUNNELED support");
        return FAILURE;
    }
