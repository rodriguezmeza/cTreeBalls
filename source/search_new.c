...
/*
 ...
 To be called using: search=octree-sincos-omp (substitute of tree-omp-sincos)
...
#endif
    if (cballs_opt_kappa_constant_one(cmd))
...
#ifdef SMOOTHPIVOT
    if (prepare_smooth_pivots(cmd, gd, btable, nbody,
...
#pragma omp parallel default(none)   \
    shared(cmd,gd,btable,nbody,roottable,ipmin,ipmax,nodetablescanlev, \
           ...
           icountNbRminOverlap, allocation_failed)
    {
#else
#pragma omp parallel default(none)   \
    shared(cmd,gd,btable,nbody,roottable,ipmin,ipmax,nodetablescanlev, \
...
#pragma omp barrier
    ...
    if (allocation_failed) {
        snprintf(cmd->error_message, _ERRORMSGSIZE_,
                 "searchcalc_balls: OpenMP histogram allocation failed");
        return FAILURE;
    }
