...
local int expandbox(struct  cmdline_data* cmd, struct  global_data* gd, bodyptr, int, int);
local int loadbody(struct  cmdline_data* cmd, struct  global_data* gd, bodyptr, int);
local int subindex(bodyptr, cellptr);
local void hackCellProp(struct  cmdline_data* cmd, struct  global_data* gd, cellptr, real, int, int);
local int setRadius(struct  cmdline_data* cmd, struct  global_data* gd, cellptr, vector, real, int);
local void threadtree(struct  cmdline_data* cmd, struct  global_data* gd, nodeptr, nodeptr);

local int findRootCenter(struct  cmdline_data* cmd, struct  global_data* gd, bodyptr, int, int);
...
// By now it is only working with boxes centered at (0,0,...)
    findRootCenter(cmd, gd, btab, nbody, ifile);
    centerBodies(btab, nbody, ifile);
    findRootCenter(cmd, gd, btab, nbody, ifile);
//E

    CLRV(Pos(roottable[ifile]));
    expandbox(cmd, gd, btab, nbody, ifile);
...
local int findRootCenter(struct  cmdline_data* cmd, struct  global_data* gd, bodyptr btab, int nbody, int ifile)
{
    real len;
    bodyptr p;
    int k;
...
local int centerBodies(bodyptr btab, int nbody, int ifile)
{
    bodyptr p;
    int k;
...
local cellptr makecell(struct  cmdline_data* cmd, struct  global_data* gd, int ifile)
{
    cellptr c;
    int i;
...
local int loadbody(struct  cmdline_data* cmd, struct  global_data* gd, bodyptr p, int ifile)
{
    cellptr q, c;
    int qind, k;
    real qsize, dist2;
