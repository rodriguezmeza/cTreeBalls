...
static void axis(metadata_buffer *b, const char *key, double lo, double hi, int bins)
{
    append(b, "\"%s\":[", key);
...
int cballs_run_metadata(struct cmdline_data *cmd, struct global_data *gd, char **result)
{
    ...
    *result = NULL;
    ...
    named_string(&b, "estimator", forest ? (radial_forest ? (strstr(method, "same-los") ? "equal-forest mean of within-forest weighted radial 2PCF" : "weighted forest radial correlations") : "weighted anisotropic forest correlations")
        : shear ? "full-sky spin-2 pair correlations and window-corrected natural 3PCF components"
        ...
        : cballs_opt_edge_corrections(cmd) ? "scalar tangent-Fourier angular window deconvolution"
        : cballs_opt_no_normalize_histzeta(cmd) ? "raw scalar distinct-neighbor tangent-Fourier moments"
        : "weight-normalized scalar tangent-Fourier moments");
    ...
    if (forest) {
        axis(&b, "two_point_abs_parallel", 0, cmd->lya2RpMax, cmd->lya2RpBins);
        if (!radial_forest) {
            append(&b, ","); axis(&b, "two_point_transverse", 0, cmd->lya2RtMax, cmd->lya2RtBins);
            append(&b, ","); axis(&b, "three_point_side", 0, cmd->lya3RMax, cmd->lya3RBins);
            append(&b, ","); axis(&b, "three_point_los_angle", 0, PI, cmd->lya3ThetaBins);
            append(&b, ","); axis(&b, "three_point_opening_cosine", -1, 1, cmd->lya3MuBins);
        } else {
            append(&b, ","); axis(&b, "three_point_signed_parallel", -cmd->lya3RMax, cmd->lya3RMax, 2*cmd->lya3RBins);
        }
    }
#endif
    ...
    string_value(&b, forest ? "observer-centered Cartesian comoving positions; radial norm/LOS separation; no periodic wrapping"
        : shear ? "unit-sphere Cartesian directions; chord-distance bins; parallel-transported spin-2 tangent components"
