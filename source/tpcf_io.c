...
int inputdata(struct cmdline_data* cmd, struct  global_data* gd, string filename, int ifile)
{
...
//#ifdef CLASSLIB
            class_call_cballs(inputdata_takahasi(cmd, gd, filename, ifile),
                       errmsg, errmsg);
//#else
//            inputdata_takahasi(cmd, gd, filename, ifile);
//#endif
            break;
                               ...
                               string filename, int ifile)
{
    ...
    gd->model_comment = "Column position input file";
...
//#if NDIM == 3
local int Takahasi_region_selection_3d_all(struct cmdline_data* cmd, struct  global_data* gd,
                                           int nside, int npix,
...
local int Takahasi_region_selection_3d(struct cmdline_data* cmd, struct  global_data* gd,
                                       int nside, int npix,
    float *conv, float *shear1, float *shear2, float *rotat,
    real dtheta_rot, real thetaL, real thetaR,
    real dphi_rot, real phiL, real phiR,
...
#if NDIM == 2
local int Takahasi_region_selection_2d(struct cmdline_data* cmd, struct  global_data* gd,
                                       int nside, int npix,
            float *conv, float *shear1, float *shear2, float *rotat,
            real dtheta_rot, real thetaL, real thetaR,
            real dphi_rot, real phiL, real phiR,
...
//    fprintf(gd->outlog,"\nsystem: %s\n",buf);
    ...
    sprintf(gd->fpfnamehistNFileName,"%s/%s%s%s",
            cmd->rootDir,cmd->histNFileName,cmd->suffixOutFiles,EXTFILES);
// Will be gd->histNFileName. Set it in gd structure
    ...
    sprintf(gd->fpfnamehistZetaMFileName,"%s/%s%s",
            cmd->rootDir,cmd->histZetaMFileName,cmd->suffixOutFiles);
    sprintf(gd->fpfnamemhistZetaFileName,"%s/%s%s",
            cmd->rootDir,cmd->mhistZetaFileName,cmd->suffixOutFiles);
...
// 2pcf
//B kappa Avg Rmin
    free_dvector(gd->histNSubXi2pcftotal,1,cmd->sizeHistN);
//E
