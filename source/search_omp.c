...
/*
 Search omp parallel/serial routine using tree method:
 ...
 Return:
    int : status of the routine run: _SUCCESS_ or _FAILURE_
 */
global int searchcalc_normal_omp_sincos(struct  cmdline_data* cmd, struct  global_data* gd,
...
//E
            computeBodyProperties_sincos_omp(cmd, gd, p, nbody[cat1], &hist);
...
//E
        ...
        if (cmd->computeTPCF) {
            for (mm=1; mm<=cmd->mchebyshev+1; mm++) {
                ...
                MULMS_ext(gd->histZetaMsincos[mm],gd->histZetaMsincos[mm],xi,cmd->sizeHistN);
            }
        }
    }
            ...
            if (cmd->useLogHist) {
                if(dr1>cmd->rminHist) {
                    ...
                    if (n<=cmd->sizeHistN && n>=1) {
                        ...
                        //B kappa Avg Rmin
                        hist->histNSubXi2pcfthreadp[n] =
                        hist->histNSubXi2pcfthreadp[n] + 1.;
                        //E
