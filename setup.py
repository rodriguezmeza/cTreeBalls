...
OCTREESHEARSPHERE2BALLSMPION = (
    "1"
    if cppflag_has_macro(make_cppflags, "OCTREESHEARSPHERE2BALLSMPI")
    else "0"
)
...
KDTREESHEARSPHERE2BALLSMPION = (
    "1"
    if cppflag_has_macro(make_cppflags, "KDTREESHEARSPHERE2BALLSMPI")
    else "0"
)
...
BALLTREESHEARSPHERE2BALLSMPION = (
    "1"
    if cppflag_has_macro(make_cppflags, "BALLTREESHEARSPHERE2BALLSMPI")
    else "0"
)
MPI_ENABLED = cppflag_has_macro(make_cppflags, "CBALLS_MPI_ENABLED")
SLEEFON = "1" if cppflag_has_macro(make_cppflags, "DUAL_NODE_USE_SLEEF") else "0"
...
sleef_rpath_args = []
if SLEEFON == "1":
    sleef_include_dirs, sleef_library_dirs, sleef_libraries = parse_pkg_config("sleef")
    if not sleef_libraries:
        raise RuntimeError(
            "DUAL_NODE_USE_SLEEF is active, but pkg-config cannot resolve sleef"
        )
    liblist += sleef_libraries
    sleef_rpath_args = [
        f"-Wl,-rpath,{directory}" for directory in sleef_library_dirs
    ]
...
cyballs_ext = Extension(
    ...
    depends=[os.path.join(root_folder, f"lib{libname}.a"),
             os.path.join(cyballs_folder, "ccyballs.pxd"),
             os.path.join(cyballs_folder, "ccyballs.pxd.in"),
